...
def test_marked_declared_sidecar_reads_identically_to_unmarked(tmp_path: Path) -> None:
    """The shared-writer marker line must not change declared-sidecar rows (#6512)."""

    rows = [
        {"episode_id": "ep-1", "planner_key": "orca", "seed": "111", "mechanism_label": "unknown"},
        {"episode_id": "ep-2", "planner_key": "goal", "seed": "112", "mechanism_label": "unknown"},
    ]
    fieldnames = sorted(rows[0])
    unmarked = tmp_path / "unmarked.csv"
    ...
    for path in (unmarked, marked):
        with path.open("w", encoding="utf-8", newline="") as handle:
            ...
            writer.writerows(rows)
    marked.write_text(
        "# AI-GENERATED NEEDS-REVIEW\n" + marked.read_text(encoding="utf-8"), encoding="utf-8"
    )

    assert _MODULE._read_declared_sidecar_csv(marked) == _MODULE._read_csv(unmarked)


def test_marked_declared_sidecar_still_unblocks_tables(tmp_path: Path) -> None:
    """A declared sidecar regenerated by the migrated #4242 writer keeps the #4305 path working.

    The migrated #4242 backfill writer emits sidecars through the shared CSV writer, whose first
    line is the review marker. The declared-sidecar read path must skip that marker and supply the
    same trace labels as the unmarked legacy sidecar.
    """
