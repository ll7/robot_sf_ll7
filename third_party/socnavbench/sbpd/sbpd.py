...
class Loader():
    def load_building(self, name, data_dir=None):
        if data_dir is None:
            data_dir = self.data_dir or get_sbpd_data_dir()  # when not using surreal
        out = {}
        out['name'] = name
    ...
    def load_building_meshes(self, building, materials_scale=1.0):
        # Mesh loading is the only generation path that needs the native Assimp/OpenGL
        # bridge; keep it out of dependency-light preflight and occupancy setup imports.
