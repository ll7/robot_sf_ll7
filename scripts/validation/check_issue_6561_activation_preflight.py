...
def validate_preflight(  # noqa: PLR0915 - linear validation of frozen contract fields
    ...
    _require(
        _finite_float(
            rule.get("maximum_realized_desired_speed_mean_m_s"),
            "maximum_realized_desired_speed_mean_m_s",
        )
        == _finite_float(
            protocol_rule["maximum_realized_desired_speed_mean_m_s"],
            "protocol maximum_realized_desired_speed_mean_m_s",
        )
        == 3.0,
        "realized desired-speed mean bound drifted from the protocol",
    )
    _require(
        _finite_float(
            rule.get("maximum_realized_desired_speed_std_m_s"),
            "maximum_realized_desired_speed_std_m_s",
        )
        == _finite_float(
            protocol_rule["maximum_realized_desired_speed_std_m_s"],
            "protocol maximum_realized_desired_speed_std_m_s",
        )
        == 3.0,
        "realized desired-speed std bound drifted from the protocol",
    )
...
def _validate_diagnostic_values(
    ...
    *,
    maximum_realized_desired_speed_mean_m_s: float,
    maximum_realized_desired_speed_std_m_s: float,
) -> None:
    ...
    _require(
        0.0 <= realized_mean <= maximum_realized_desired_speed_mean_m_s,
        "realized desired-speed mean is out of range",
    )
    _require(
        0.0 <= realized_std <= maximum_realized_desired_speed_std_m_s,
        "realized desired-speed std is out of range",
    )
...
def _validate_row(
    ...
    *,
    ...
    if row_status == "native":
        _validate_diagnostic_values(
            row,
            expected_identity,
            maximum_realized_desired_speed_mean_m_s=maximum_realized_desired_speed_mean_m_s,
            maximum_realized_desired_speed_std_m_s=maximum_realized_desired_speed_std_m_s,
        )
...
def classify_activation(
    ...
    maximum_realized_desired_speed_mean_m_s = _finite_float(
        rule["maximum_realized_desired_speed_mean_m_s"],
        "maximum_realized_desired_speed_mean_m_s",
    )
    maximum_realized_desired_speed_std_m_s = _finite_float(
        rule["maximum_realized_desired_speed_std_m_s"],
        "maximum_realized_desired_speed_std_m_s",
    )
