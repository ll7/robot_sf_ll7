...
4. **Candidate-Level Outcome Admission (`adversarial_independent_outcomes.v2`)**: The frozen candidate manifest is content-addressed and must contain exactly the configured candidates per arm. Every candidate must execute each of its five unique frozen seeds exactly once; replay signatures and failure-attribution evidence must agree. The runner reduces those rows to one binary certified failure yield only after four matching confirmations, before computing metrics, null tests, or the #2921 decision. Fallback or degraded execution statuses fail closed.
