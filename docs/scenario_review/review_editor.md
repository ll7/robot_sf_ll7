...
`review-editor.v1.json` model, a self-contained HTML shell, the dependency-free
`components/review_editor/review_editor.js` module, and a capability report.
...
expected revisions. The `PersistenceAdapter` protocol is the explicit seam for
BA-05's future service; BA-05 is not merged here and SREV-17 does not invent a
second store. `ReviewEditorSession` reports `pending`, `saved`, or `error`
autosave state. A stale revision preserves the local record and includes the
remote record and selection revision in `SaveConflict`; it is never a silent
overwrite. Browser saves use an injected service callback and never
`localStorage`.
