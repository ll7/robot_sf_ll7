...
## Campaign caveats and limitations

Admitted as nominal benchmark evidence for simulator-defined social-compliance metric-family paired effects only (job-13985, campaign_integrity.status=valid). The following caveats are recorded from the retrieved campaign artifacts and are not claims:

- Positive-margin route-clearance warnings (informational; the fail-closed threshold is a margin below 0.0 m): `classic_doorway_medium` min clearance margin 0.012233 m; `classic_overtaking_medium` min clearance margin 0.326555 m.
- SNQI contract status `fail` with enforcement `warn`; positioning recommendation is downgrade to appendix/implementation aid. SNQI is reported diagnostic-only and excluded from this report's inference.
- The fairness contract excludes all three planners (goal, social_force, orca) from ranking claims (`ranking_claim_allowed=false`). No ranking claim is made.
- `release_tag` and `doi` fields remain placeholders.
- No fairness, ethics, safety, social-validity, real-world, universal-ranking, composite-score, welfare, or dissertation claim is made from this artifact.
