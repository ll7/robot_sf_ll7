...
| `hybrid_rule_local_planner` | `docs/context/policy_search/validation/2026-04-30_hybrid_rule_v0_minimal_iteration.md` | V0 passes open sanity but fails nominal sanity with static-collision termination, low-progress timeouts, and intrusive near misses. | Show nominal-sanity and stress-slice gains without increasing collisions or severe timeout/freezing. | Add one mechanism at a time, starting with static/corridor handling before ORCA/VO, recovery, TEB-like, or ensemble variants. |
