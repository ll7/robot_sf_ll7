...
> This file preserves the previously published procedural reference for compatibility. Start at
> [`docs/dev_guide.md`](./dev_guide.md) for the concise first-use path, or use
> [`docs/developer-guide.md`](./developer-guide.md) to navigate the canonical topic guides. New
> procedures should be added to a task-focused owner and linked from the index rather than added
> here.
...
Projects Classic field (`repository.pullRequest.projectCards`) and can even exit
`0` while emitting the error and no usable content (issue #6496, observed while
reviewing #6454). Neither operation needs Projects Classic data, so perform them
...
`dispatch-ownership` job elects the oldest active run for the exact main SHA.
Followers wait for that owner: they mirror only a completed exact-head
success/failure, or take ownership if the prior run becomes stale or
