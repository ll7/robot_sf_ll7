...
class SNQISensitivityAnalyzer:
    ...
    def weight_sweep_analysis(self, base_weights: Dict[str, float], 
        ...
        for weight_name in self.weight_names:
            ...
            base_scores = [self.compute_snqi(ep.get('metrics', {}), base_weights) for ep in self.episodes]
            ...
            for weight_val in weight_values:
                ...
                # Compute SNQI scores
                scores = [self.compute_snqi(ep.get('metrics', {}), modified_weights) for ep in self.episodes]
    ...
    def pairwise_weight_analysis(self, base_weights: Dict[str, float],
                               weight_pairs: List[Tuple[str, str]] = None,
                               n_points: int = 15) -> Dict[str, Any]:
    ...
    def ablation_analysis(self, base_weights: Dict[str, float]) -> Dict[str, Any]:
        ...
        for weight_name in self.weight_names:
            ...
            ablated_scores = [self.compute_snqi(ep.get('metrics', {}), ablated_weights) for ep in self.episodes]
    ...
    def normalization_strategy_analysis(self, base_weights: Dict[str, float]) -> Dict[str, Any]:
        ...
        # Create alternative baseline stats using different percentiles
        alt_baseline_stats = {}
    ...
    def _plot_weight_sweeps(self, sweep_data: Dict, output_dir: Path):
        ...
        for i, (weight_name, data) in enumerate(sweep_data.items()):
            ...
            ax.fill_between(data['weight_values'], 
            ...
            ax2 = ax.twinx()
            ax2.plot(data['weight_values'], data['ranking_correlation'], 'r--', linewidth=2, label='Ranking Corr')
    ...
    def _plot_pairwise_analysis(self, pairwise_data: Dict, output_dir: Path):
        ...
        for i, (pair_name, data) in enumerate(pairwise_data.items()):
            ...
            axes[1, i].set_title(f'Ranking Stability: {data["weight1_name"]} vs {data["weight2_name"]}')
    ...
    def _plot_normalization_analysis(self, norm_data: Dict, output_dir: Path):
        ...
        plt.tight_layout()
        plt.savefig(output_dir / 'normalization_strategy_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    parser.add_argument('--pairwise-points', type=int, default=15,
    ...
    if 'normalization' in results:
        ...
        for strategy, data in norm_results.items():
            if strategy != base_strategy:
                corr_change = abs(data['ranking_correlation'] - norm_results[base_strategy]['ranking_correlation'])
