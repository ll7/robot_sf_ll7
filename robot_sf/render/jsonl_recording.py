...
class JSONLRecorder:
    ...
    def _get_episode_filename(self, episode_id: int) -> Path:
        ...
        filename = f"{self.suite}_{self.scenario}_{self.algorithm}_{self.seed}_ep{episode_id:04d}.jsonl"
    ...
    def _serialize_state(self, state: VisualizableSimState) -> Dict[str, Any]:
        ...
        if hasattr(state, 'pedestrian_positions') and state.pedestrian_positions is not None:
            # Handle numpy array
            if hasattr(state.pedestrian_positions, '__len__') and len(state.pedestrian_positions) > 0:
