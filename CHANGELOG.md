...
* Added the issue-856 broad-training campaign surfaces: seed-replica training configs (`expert_ppo_issue_791_all_scenarios_10m_env22_large_capacity_seed{231,1337}.yaml`), the 12223 baseline adapter (`configs/baselines/ppo_issue_856_all_scenarios_12223.yaml`), the dedicated camera-ready comparison matrix (`configs/benchmarks/paper_experiment_matrix_v1_issue_856_all_scenarios_compare.yaml`), and the campaign and pre-PR verification context notes. Recorded the verdict that, at fixed 10M budget, broad-training underperforms the eval-aligned leader on the camera-ready matrix (success −0.035, collisions +0.007, SNQI −0.040), strengthening the alignment-vs-diversity claim for the manuscript.
* Made the issue-791 SLURM wrappers worktree-safe: shared `run_in_allocation` srun fallback in `issue_791_{asymmetric_critic,attention_head,reward_curriculum}.sl`, plus `issue_791_benchmark.sl` now symlinks the Wave-5 leader cache from the primary checkout when the worktree is missing it and gates the leader-artifact check on whether the benchmark config actually references it.
...
* Added a Codex-native repository context stack: strengthened `AGENTS.md`, new `docs/code_review.md` benchmark/provenance checklist, `.agent/PLANS.md` planning convention, repo-local context skills under `.codex/skills/`, and AI-facing overview/decision docs under `docs/ai/`.
