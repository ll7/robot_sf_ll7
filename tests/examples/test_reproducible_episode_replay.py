...
_EXAMPLE_PATH = (
    ...
    / "examples"
    / "advanced"
    / "43_reproducible_episode_replay.py"
)
...
def test_report_distinguishes_same_and_changed_seed(
    ...
    def run_record(seed: int, max_steps: int) -> EpisodeRecord:
        """Return a fresh record on every simulated public-facade execution."""
        del max_steps
        calls[seed] = calls.get(seed, 0) + 1
        return _record(seed, float(calls[seed]))

    monkeypatch.setattr(module, "_run_record", run_record)

    parent_entries_before = {entry.name for entry in tmp_path.parent.iterdir()}
    ...
    assert {entry.name for entry in tmp_path.iterdir()} == expected_children
    assert all(entry.is_file() for entry in tmp_path.iterdir())
    assert {entry.name for entry in tmp_path.parent.iterdir()} == parent_entries_before
    loaded_records = {
        label: EpisodeRecord.load(tmp_path / f"{label}.json")
        for label in ("same_seed_a", "same_seed_b", "different_seed")
    }
    assert (
        report["comparisons"]["same_seed"]["first_digest"]
        == loaded_records["same_seed_a"].canonical_digest()
    )
    assert (
        report["comparisons"]["same_seed"]["second_digest"]
        == loaded_records["same_seed_b"].canonical_digest()
    )
    assert (
        report["comparisons"]["different_seed"]["second_digest"]
        == loaded_records["different_seed"].canonical_digest()
    )
    assert all(
        artifact["canonical_digest"] == loaded_records[artifact["label"]].canonical_digest()
        for artifact in report["artifacts"]
    )
