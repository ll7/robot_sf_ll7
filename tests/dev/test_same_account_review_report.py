...
def test_markerless_edit_of_newer_report_supersedes_older_acceptance() -> None:
    older = _app_comment(_report_body(), comment_id=100, created_at="2026-09-10T10:00:00Z")
    newer = _app_comment(_report_body(), comment_id=101, created_at="2026-09-10T10:01:00Z")
    newer["body"] = "Edited after publication; previous review withdrawn."
    newer["updated_at"] = "2026-09-10T10:02:00Z"

    reports, provenance = project_static_reports_from_comments(
        [older, newer], repository=REPO, pr_number=PR
    )

    assert provenance["candidate_count"] == 2
    assert provenance["selected_comment_id"] == 101
    assert reports[0]["superseded"] is True
    assert reports[1]["custody"]["reason_code"] == "static_report_comment_edited"
    assert _classify(reports)["status"] == "superseded"
