...
- The activation diagnostics contract records `time_to_desired_speed_target_seconds` as a row-level p95 in seconds: the 95th percentile across pedestrians of elapsed time to first target-tolerance entry after the declared transient exclusion. It is not a mean, maximum, or per-pedestrian sample; the inclusive activation limit is `2.0 s`. The realized desired-speed standard deviation is bounded to `0–3.0 m/s`, using the existing inclusive `3.0 m/s` upper clip of the shared sampler. These are conservative structural/arithmetic bounds, not runtime or benchmark results.
