...
def _scrub_volatile(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in payload.items():
        ...
        if key == "source_files" and isinstance(value, list):
            ...
            continue
        # Deep-copy remaining values so the scrubbed comparison payload never
        # aliases nested structures in the source metadata.
        scrubbed[key] = copy.deepcopy(value)
    return scrubbed
...
def test_canonical_table_golden_columns_match_spec_contract() -> None:
    ...
    for fmt in _CANONICAL_TABLE_GOLDEN_PATHS:
        path = _CANONICAL_TABLE_GOLDEN_PATHS[fmt]
        if _is_bless() and not path.is_file():
            # Goldens may not exist yet during a fresh bless run; the surface
            # tests above regenerate them. Don't crash the contract check.
            continue
        ...
        if fmt == "csv":
            ...
            # LaTeX escapes underscores in the tabular header row.
            header = text.splitlines()[3]
            assert all(column.replace("_", r"\_") in header for column in spec.columns)
...
def test_parquet_goldens_cover_all_exported_tables() -> None:
    ...
    if _is_bless():
        # During a fresh bless run the goldens are (re)generated by the surface
        # test; skip the coverage assertion rather than fail before they exist.
        return
    for name in _PARQUET_TABLE_NAMES:
...
def test_scrub_volatile_does_not_alias_nested_source_values() -> None:
    """The scrubbed payload must deep-copy nested values, not alias the source."""
    payload = {"nested": {"rows": [{"k": 1}]}}
    scrubbed = _scrub_volatile(payload)
    scrubbed["nested"]["rows"][0]["k"] = 999
    # Mutating the scrubbed copy must not leak back into the source payload.
    assert payload["nested"]["rows"][0]["k"] == 1
