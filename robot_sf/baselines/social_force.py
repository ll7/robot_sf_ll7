...
class SocialForcePlanner(BasePolicy):
    ...
    def step(self, obs: Union[Observation, Dict]) -> Dict[str, float]:
        ...
        total_force = self._compute_total_force(
            robot_pos, robot_vel, robot_goal, robot_radius, obs
        )
        
        # Convert force to action
        action = self._force_to_action(
            total_force, robot_pos, robot_vel, robot_goal, dt
        )
    ...
    def _setup_simulation(self, obs: Union[Observation, Dict]) -> None:
        ...
        if n_agents > 0:
            ...
            ped_state = PedState(
                ...
                vel=velocities,
                goal=goals,
                max_speeds=np.full(n_agents, self.config.desired_speed),
                agent_radius=np.full(n_agents, 0.35),  # default ped radius
            )
        else:
            # Empty pedestrian state
            ped_state = PedState(
                pos=np.zeros((0, 2)),
                vel=np.zeros((0, 2)),
                goal=np.zeros((0, 2)),
                max_speeds=np.array([]),
                agent_radius=np.array([]),
            )
    ...
    def _force_to_action(
        ...
        if self.config.action_space == "velocity":
            ...
            return self._force_to_unicycle_action(
                force, robot_pos, robot_vel, robot_goal, dt
            )
        else:
