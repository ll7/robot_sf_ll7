...
- **Destructive restores need a backup or a clean tree**: Never run bare `git checkout -- <paths>` in a checkout whose `git status` is dirty. Route the restore through `scripts/dev/safe_checkout.sh -- <checkout-args>`, which refuses on tracked dirt unless `--backup <patch-file>` (or `--path` scoping) captured it first (issue #9445).
...
- **Explicit issue relationships**: Issue and PR workflows read [`issue_relationships.md`](../context/issue_relationships.md). Native Parent/Blocked by/Blocking links are canonical; bodies and comments provide evidence, not a duplicate graph. Set links only from the owning writable worktree; review-only worktrees verify fresh native state and never write edges. Direct CLI relationship operations require `gh >= 2.100.0`; unsupported fields use the documented fallback and fail closed.
