...
def build_gap_prediction_config(cfg: dict[str, Any] | None) -> GapPredictionConfig:
    ...
    gap_cfg = StreamGapPlannerConfig(
        max_linear_speed=float(gap_raw.get("max_linear_speed", DEFAULT_MAX_LINEAR_SPEED)),
        max_angular_speed=float(gap_raw.get("max_angular_speed", DEFAULT_MAX_ANGULAR_SPEED)),
        ...
        forward_lookahead=float(gap_raw.get("forward_lookahead", DEFAULT_FORWARD_LOOKAHEAD_M)),
        ...
        sample_horizon=float(gap_raw.get("sample_horizon", DEFAULT_SAMPLE_HORIZON_S)),
