...
Advance the two smoke-passing candidates into `nominal_sanity` and validate one concrete tuning attempt on the best unfinished hybrid candidate.
...
- `scenario_adaptive_orca_v1` nominal sanity:
  ...
  - near miss: `0.1111`
  - decision: `revise`
  - dominant failures: `timeout_low_progress=12`, `near_miss_intrusive=2`
...
`scenario_adaptive_orca_v1` is the safer nominal-sanity candidate because it stays collision-free, but its success rate is still far below the `0.80` gate and it stalls across both classic and Francis subsets.

## Next Step

Prioritize tuning `scenario_adaptive_orca_v1` for progress without giving back its current collision advantage. Keep `hybrid_orca_sampler_v1` as a secondary tuning track until per-step diagnostics explain why the ORCA head still fails the simplest sanity route even after the handoff fix.