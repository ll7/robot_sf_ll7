...
def _evidence_receipt(
    *,
    ...
    tracking_epoch_id: str = "epoch-1",
    timestamp_s: float = 1.5,
    step_index: int = 3,
    candidate_set_digest: str = POSTERIOR_CANDIDATE_SET_DIGEST,
    evidence_source: str = "upstream_selected",
    producer: str = "public_fixture",
) -> HierarchicalEvidenceReceiptV1:
    """Build an explicit actor-safe receipt for the synthetic evidence packet."""
    return HierarchicalEvidenceReceiptV1(
        schema_version=HIERARCHICAL_EVIDENCE_RECEIPT_SCHEMA_VERSION,
        ...
        packet_schema_version="synthetic_hierarchical_evidence.v1",
    )


def _rebind_candidate_set(
    posterior: HierarchicalGoalPosteriorV1,
    candidate_set: GoalCandidateSet,
) -> HierarchicalGoalPosteriorV1:
    """Rebind both posterior and receipt to a replacement candidate set."""
    return _rebind_candidate_set_digest(
        posterior,
        stable_digest(candidate_set.to_dict()),
    )


def _rebind_candidate_set_digest(
    posterior: HierarchicalGoalPosteriorV1,
    candidate_set_digest: str,
) -> HierarchicalGoalPosteriorV1:
    """Rebind both posterior and receipt to one candidate-set digest."""
    assert posterior.evidence_receipt is not None
    return replace(
        posterior,
        candidate_set_digest=candidate_set_digest,
        evidence_receipt=replace(
            posterior.evidence_receipt,
            candidate_set_digest=candidate_set_digest,
        ),
    )
...
def _posterior(
    *,
    ...
    return HierarchicalGoalPosteriorV1(
        ...
        evidence_receipt=(
            _evidence_receipt(evidence_source=evidence_source)
            if evidence_source == "upstream_selected"
            else None
        ),
    )
...
def test_upstream_selected_requires_an_explicit_evidence_receipt() -> None:
    """Synthetic or stale evidence cannot be relabeled as admitted upstream data."""
    with pytest.raises(ValueError, match="evidence_receipt"):
        replace(_posterior(), evidence_receipt=None)


def test_evidence_receipt_binds_causal_identity_and_candidate_set() -> None:
    """Receipt metadata must match the posterior's causal and candidate bindings."""
    posterior = _posterior()
    assert posterior.evidence_receipt is not None
    ...
    with pytest.raises(ValueError, match="step_index"):
        replace(
            posterior,
            evidence_receipt=replace(posterior.evidence_receipt, step_index=2),
        )
    with pytest.raises(ValueError, match="candidate_set_digest"):
        replace(posterior, candidate_set_digest="c" * 64)


def test_evidence_receipt_digest_and_admission_fields_are_strict() -> None:
    """Receipt tampering, invalid status, and invalidity cannot pass parsing."""
    receipt = _evidence_receipt()
    payload = receipt.to_dict()
    payload["evidence_digest"] = "c" * 64
    with pytest.raises(ValueError, match="receipt_digest"):
        HierarchicalEvidenceReceiptV1.from_dict(payload)

    with pytest.raises(ValueError, match="admission_status"):
        replace(receipt, admission_status="diagnostic")
    with pytest.raises(ValueError, match="valid must be true"):
        replace(receipt, valid=False)
...
def test_flat_projection_recomputes_and_checks_candidate_set_digest() -> None:
    ...
    posterior = _rebind_candidate_set_digest(_posterior(), "c" * 64)
...
def test_flat_projection_rejects_unreferenced_candidate_ids() -> None:
    ...
    posterior = _rebind_candidate_set(_posterior(), candidate_set)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "routeTruth",
        "assignedRoute",
        "sim_pedestrian_id:42",
        "simPedestrianId:42",
        "force_component:desired",
...
@pytest.mark.parametrize(
    "destination_id",
    ["true_route", "trueRoute", "routeTruth", "sim_pedestrian_id:42", "simPedestrianId:42"],
)
...
@pytest.mark.parametrize(
    "blocker",
    [
        "oracle goal: true_goal",
        "sim_pedestrian_id:42",
        "force component: desired",
        "forceComponent:desired",
        "speed_cap_truth",
        "uncapped_velocity_xy",
    ],
