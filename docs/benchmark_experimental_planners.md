...
| `brne` | `docs/context/issue_6464_brne_corridor_diagnostic.md`, `docs/context/issue_5318_brne_integration_decision.md`, `docs/context/issue_5311_brne_source_smoke.md` | Bounded corridor diagnostic completed: pinned native BRNE execution was exact and non-degenerate on 3/3 seeds with 0/3 goal-reaching; no corridor violations or fallback/degraded rows. This is diagnostic evidence only, not a benchmark result. | Explain or revise the zero-goal-reaching behavior before proposing any broader or ranked campaign. | Run a hypothesis-driven native corridor diagnostic first; retain fail-closed fallback, native-only provenance, and the corridor-only scope. |
