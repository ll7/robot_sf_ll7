...
- **Config**: a `scene` mapping with optional `frame_indices` (distinct
  ...
  `height_in`, `dpi`; default is a small 3.2x2.4 figure at 80 dpi). Requests
  are bounded to 16 sources, 16 MiB per source, 4,096 source frames, 256
  rendered frames, 2,048 pedestrians per rendered frame, and a 12x12 inch /
  300 dpi figure budget.
...
symlink escapes, unsafe/duplicate source IDs, special-file sources, and
oversized requests are rejected. The source map records the SHA-256 of the
...
attestation. The v1 request schema optionally carries `sha256`, `source_commit`,
`config_identity`, `units`, and `coordinate_frame` declarations: a matching
source SHA is verified, while declarations this leaf cannot verify are
diagnosed and the source is not rendered. Malformed JSON object shapes at the
CLI/API boundary return a schema-valid failed result, including nested scene
configuration errors. The CLI result envelope includes
`schema_version: component-result.v1` and every result exposes
`evidence_boundary: analysis_workbench_only`, `diagnostic_only: true`,
`admission: not_evaluated`, and the conservative claim boundary
`diagnostic_only; not benchmark evidence`.

The source map declares `review-scene-sourcemap.v1`, but a checked-in
review-scene/source-map JSON Schema and publish-time validator remain an
SREV-01/shared-contract-owner follow-up. This leaf does not widen the shared
schema owner or treat the unvalidated manifest shape as independent evidence.
SVG/PDF/PNG metadata and SVG identifiers are pinned so repeated fixture runs
compare equal artifact bytes and logical digests. SVG output carries fixture
coordinates as figure text; PNG dimensions follow the figure preset.
