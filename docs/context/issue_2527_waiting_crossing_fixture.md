...
`waiting_then_crossing` phases. When a scenario or single pedestrian opts into
...
Force behavior, certify the fixture for benchmark comparisons, or support planner-ranking claims.
...
fields or a runtime snapshot that records release timing and role provenance outside map-runner's
analysis-only metadata. Data-grounded intent realism and planner-ranking claims should remain
separate issues with their own evidence targets.
