...
Historical fail-closed decision packet for issue #5034 ("execute the
control-action-latency fidelity sweep"), captured at base commit `e128d3fa2`.
This is a **launch/readiness artifact only** — it is not
...
**PR #5026** (parent issue #4977). It was absent at the recorded base commit,
so that historical preflight failed closed:
...
The fixed-scope runner was launchable on this host at the recorded base commit,
but only over the five fidelity axes present then — it had nothing to do with
control-action latency. Observed on `imech156-u` at base commit `e128d3fa2`:
...
# Historical result at e128d3fa2: decision: blocked (exit 2).
```

The `git_head` and `date` fields vary by checkout/run; all other fields are
deterministic for a given config. A checkout containing the axis should instead
report `decision: ready`; that remains readiness proof only, not campaign
evidence.

## Next empirical action (post-#5026)

1. Re-run the preflight above after the `control_action_latency` axis and
   `sim_config.action_latency_steps` are present; expect `decision: ready`,
   `observed_latency_steps: [0, 1, 3]`.
2. Preflight the fixed-scope plan
   ...
   implementation lane.
3. Promote the compact latency-cell evidence (success / collision /
