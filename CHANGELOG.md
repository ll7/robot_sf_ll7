...
* DreamerV3 RLlib training artifacts now persist `episode_len_mean` so run summaries can distinguish short episodes from short per-iteration sampling budgets.
* DreamerV3 scenario-matrix env construction now preserves occupancy-grid observation settings, fixing socnav-grid parity for benchmark-matrix training and evaluation.
* Tuned the BR-08 full socnav-grid Dreamer profile to use explicit local-learner GPU placement with fewer env runners and smaller Dreamer sequence batches so 8 GiB GPUs avoid immediate learner OOMs.
...
* Added a higher-throughput BR-08 Dreamer socnav-grid full profile tuned for the current 32-CPU/1-GPU machine, with pinned env runners, larger rollout fragments, and benchmark-matrix evaluation.
