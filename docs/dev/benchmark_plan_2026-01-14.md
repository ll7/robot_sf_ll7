...
- `robot_sf/planner/socnav.py` includes lightweight SocNav-style adapters (Sampling, Social Force, ORCA-like, SA-CADRL-like) and a fallback adapter that uses upstream SocNavBench if `thrid_party/socnavbench` is available.
...
- `thrid_party/socnavbench` includes a vendored subset of the upstream repository (planner + dependencies). The upstream sampling planner can be used via `SocNavBenchSamplingAdapter`.
...
- SocNavBench vendored subset: `thrid_party/socnavbench/`
