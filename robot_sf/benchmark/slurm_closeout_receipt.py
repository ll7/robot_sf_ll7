...
The scheduler is the authority for execution state.  A queue or admission
record is not sufficient after a terminal readback.  This packaged module is
the runtime contract used by the release doctor; private-ops may produce the
JSON without importing this checkout.
"""
...
def _normalise_state(value: object) -> str:
    ...
    return text.upper().split()[0] if text else ""
...
def validate_payload(  # noqa: C901, PLR0912, PLR0915
    ...
    *,
    ...
    for field, message in (
        ("tool", "closeout receipt query tool is missing"),
        ("tool_version", "closeout receipt query tool version is missing"),
        ("queried_at", "closeout receipt query timestamp is missing"),
    ):
...
def validate_file(
    ...
    *,
    ...
    Returns:
        Sanitized validation problems; an empty list means that the receipt is valid.
    """
    try:
