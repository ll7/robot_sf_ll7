...
| `brne` | `docs/context/issue_6464_brne_corridor_diagnostic.md`, `docs/context/issue_6934_brne_action_weight_contract.md`, `docs/context/issue_5318_brne_integration_decision.md`, `docs/context/issue_5311_brne_source_smoke.md` | The pinned upstream contract requires a mean-normalized weighted mean over control samples; the adapter's prior weighted sum was repaired without changing safety limits. The exact three-seed intervention is native and diagnostic-only: 0/3 goal-reaching, 0/3 collisions, and timeout/deadlock diagnostics in all three rows. | Recover goal-reaching with a separately justified progress/control hypothesis while preserving the upstream contract, fail-closed runtime provenance, and corridor-only scope. | Do not broaden or rank BRNE from this result. Keep the testing-only guard and retain native-core-via-adapter and diagnostic-only labeling. |
