...
def _run(args: list[str]) -> subprocess.CompletedProcess:
    """Run a command with captured output and the SNQI light-test fast path disabled.
...
def test_bootstrap_ci_interval_contains_mean(tmp_path: Path, cmd: str):
    """Verify the bootstrap CI contains the mean of the bootstrap means.

    Invokes `snqi <cmd>` with 7 bootstrap samples and 0.90 confidence, then
    checks the reported CI bounds are finite and ordered, std_mean is
    non-negative and finite, and mean_mean lies within the interval (1e-12 slack).
