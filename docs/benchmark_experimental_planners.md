...
| `hybrid_rule_local_planner` | `docs/context/policy_search/reports/2026-04-30_best_non_learning_local_policy_report.md` | Best current hybrid-rule configuration is `hybrid_rule_v3_static_margin0`: collision-free on nominal sanity and better than v0, but still below promotion bar due low-progress timeouts and intrusive near misses. | Show stress-slice gains without increasing collisions or severe timeout/freezing. | Investigate route-progress commitment, doorway comfort, and dynamic-agent passage logic before adding broader recovery or ensemble claims. |
