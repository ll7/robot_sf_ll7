...
def test_hatch_vcs_describe_command_ignores_artifact_tags(tmp_path: Path) -> None:
    """Non-version release tags must not poison hatch-vcs package version discovery."""
    ...
    default_describe = git("describe", "--dirty", "--tags", "--long", "--abbrev=40")
    assert default_describe.stdout.startswith("artifact/legacy-models-")

    pyproject = tomllib.loads((REPO_ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    command = pyproject["tool"]["hatch"]["version"]["raw-options"]["scm"]["git"]["describe_command"]
    if isinstance(command, str):
        command = shlex.split(command)
    assert command[:2] == ["git", "describe"]
