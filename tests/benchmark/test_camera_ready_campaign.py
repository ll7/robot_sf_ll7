...
def test_campaign_refactor_functions_remain_focused() -> None:
    ...
    focused_function_names = (
        ...
        "_split_planner_rows_by_group",
        "_write_campaign_table_artifacts",
        "_write_breakdown_and_parity_artifacts",
        "_write_parity_table",
        "_compute_campaign_outcome_state",
        "_resolve_snqi_baseline_and_weights",
        "_validate_snqi_inputs_if_paper_facing",
        "_compute_snqi_hashes",
        "_determine_snqi_contract_warnings",
        ...
        "_build_release_artifact_urls",
        "_build_campaign_artifacts_section",
        "_build_run_meta_preflight_artifacts",
        "_build_run_meta_seed_variability_metrics",
        "_build_run_meta",
        "_build_campaign_manifest_payload",
        "_write_run_level_files",
        "_export_publication_bundle_section",
        "_write_final_campaign_artifacts",
        "_write_table_and_breakdown_artifacts",
        "_write_diagnostic_sections",
        "_assemble_report_artifacts",
        "_write_campaign_report_artifacts",
        "_build_summary_and_write_run_files",
        "_export_and_write_final_artifacts",
        "_finalize_campaign_outputs",
        "_build_table_paths_dict",
...
def test_campaign_integrity_resolves_relocated_episode_paths(tmp_path: Path) -> None:
    """Relocated campaigns resolve their episode paths via local 'runs' subpath fallback."""
    relocated_root = tmp_path / "relocated_campaign"
    runs_dir = relocated_root / "runs" / "goal"
    ...
    # but contains "runs/goal/episodes.jsonl"
    non_existent_abs_path = "/home/user/old_workspace/output/benchmarks/camera_ready/my_campaign/runs/goal/episodes.jsonl"
...
class TestBuildScenarioAmvLookup:
    ...
    def test_build_scenario_archetype_lookup_allows_identical_normalized_duplicates(self) -> None:
        """Repeated normalized IDs are valid when their archetype declarations agree."""
        lookup = _build_scenario_archetype_lookup(
            [
                {"name": " shared ", "metadata": {"archetype": " bottleneck "}},
                {"scenario_id": "shared", "metadata": {"archetype": "bottleneck"}},
            ]
        )
        assert lookup == {"shared": "bottleneck"}

    def test_build_scenario_archetype_lookup_rejects_conflicts_regardless_of_order(self) -> None:
        """Conflicting normalized IDs fail closed with an order-independent diagnostic."""
        scenarios = [
            {"name": " shared ", "metadata": {"archetype": "bottleneck"}},
        ...
        messages = []
        for ordered_scenarios in (scenarios, list(reversed(scenarios))):
            with pytest.raises(ValueError) as exc_info:
                _build_scenario_archetype_lookup(ordered_scenarios)
            messages.append(str(exc_info.value))

        assert messages[0] == messages[1]
        assert "shared" in messages[0]
        assert "bottleneck" in messages[0]
        assert "crossing" in messages[0]

    def test_build_scenario_archetype_lookup_rejects_tagged_and_untagged_duplicates(
        self,
    ) -> None:
        """A missing declaration is also ambiguous when its duplicate is tagged."""
