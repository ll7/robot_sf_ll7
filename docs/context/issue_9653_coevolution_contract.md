...
optimizer/falsification budgets, base seeds, held-out IDs, and the named
optimizer/template/search-space inputs. Every round derives and records the
exact seeds, case IDs, budgets, and source revision before invoking an adapter.
Files referenced by those configs, such as map or route assets, must also be
listed in `additional_input_files` when their digests are not already recorded
by the corresponding owner manifest.

## Evidence and admission rules

Each owner phase writes a separate machine-readable result with an input digest,
output digest, and file digest. Resume verifies the config, named input files,
source revision, non-output working-tree bytes, Python/platform identity, and
completed phase artifacts, including the selected planner config path and bytes.
...
simulator work; its diagnostic remains in the run manifest.
...
fallback/degraded/failed rows, optimizer improvement and plateau, round-budget
exhaustion, infrastructure failure, and resume digest validation without
re-running completed phases. These fixtures do not establish planner or search
performance.
