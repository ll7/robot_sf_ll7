...
robot/pedestrian state, metric row, denominator status, artifact status, provenance fields, and
fail-closed figure eligibility.
...
uv run python scripts/analysis/build_signalized_crossing_failure_pack_issue_2754.py \
  ...
  --trace-source-kind fixture \
  --metric-source-kind synthetic \
  --evidence-tier smoke \
  --execution-mode fixture \
  --claim-matrix-status diagnostic_only \
