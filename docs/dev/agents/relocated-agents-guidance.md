# Relocated Agent Guidance

This file preserves long-form guidance moved out of `AGENTS.md` for issue #4461. The move is intended to reduce boot-token overhead; rules here remain authoritative when the linked situation applies.
...
- The legacy issue-to-PR names `gh-issue-autopilot`, `issue-to-pr`, and `gh-issue-to-pr` resolve to
  the selected-issue mode in `.agents/skills/goal-issue-implementation/SKILL.md`.
