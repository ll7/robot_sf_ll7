...
2. **Required checkpoints resolve AND exist.** A ``model_id`` is accepted when its registry
   ...
   so this check deliberately mirrors
   :func:`robot_sf.benchmark.campaign.campaign_checkpoint_preflight`
...
# Mirrors :data:`robot_sf.benchmark.campaign.campaign_checkpoint_preflight._REMOTE_SOURCE_KEYS` to
# avoid a
...
def _entry_has_remote_source(entry: dict[str, Any]) -> bool:
    ...
    Mirrors :func:`robot_sf.benchmark.campaign.campaign_checkpoint_preflight._entry_has_remote_source`:
...
def _iter_model_ids(algo_config: dict[str, Any]) -> list[str]:
    ...
    :data:`robot_sf.benchmark.campaign.campaign_checkpoint_preflight._CHECKPOINT_REFERENCE_KEY_PAIRS`
    so a
