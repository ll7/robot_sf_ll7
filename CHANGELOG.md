...
* Camera-ready planner rows now expose `most_likely_failure_reason` and campaign reports include a dedicated “Failed Planners And Likely Reasons” section; the all-planners paper-facing config for tag `0.0.2` now uses `stop_on_failure: false` so one planner failure is explicit without aborting the full campaign.
* Added first scoped benchmark release v0.0.2 (7-planner release excluding socnav_bench due to missing external SocNavBench dataset dependency): new campaign config `paper_experiment_matrix_7planners_v1.yaml`, release manifest `paper_experiment_matrix_7planners_v1_release_v0_0_2_scoped.yaml`, and durable rationale note in `docs/context/benchmark_release_0_0_2_scoped_rationale.md` with full historical record of blockage and future-path guidance for upgrading to full 8-planner release once assets are staged.
