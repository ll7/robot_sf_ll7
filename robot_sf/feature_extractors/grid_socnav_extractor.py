...
def _init_classes() -> dict[str, Any]:  # noqa: C901
    ...
    class PedestrianAttentionHead(nn.Module):
        ...
        def forward(
            ...
            if count is not None:
                count_int = count.view(-1).long()
                idx = th.arange(max_peds, device=slot_feats.device).unsqueeze(0)
                key_padding_mask = idx >= count_int.unsqueeze(1)
            ...
            pooled = (x * valid).sum(dim=1) / valid_count
    ...
    class GridSocNavExtractor(BaseFeaturesExtractor):
        ...
        def __init__(  # noqa: C901,PLR0912,PLR0913,PLR0915
            ...
            *,
            ...
            if use_pedestrian_attention:
                ...
                resolved_slot_keys = [
                    k for k in resolved_slot_keys if k in observation_space.spaces
                ]
                ...
                slot_input_dim = int(
                    sum(
                        int(observation_space.spaces[k].shape[-1])
        ...
        @staticmethod
        def _infer_grid_output_dim(
            grid_extractor: nn.Sequential, grid_shape: tuple[int, ...]
        ) -> int:
    ...
    return {
        "PedestrianAttentionHead": PedestrianAttentionHead,
        "GridSocNavExtractor": GridSocNavExtractor,
    }


_cache: dict[str, Any] | None = None
_LAZY_NAMES = {"PedestrianAttentionHead", "GridSocNavExtractor"}
