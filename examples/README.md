...
| [06 Pedestrian Env Factory](./advanced/06_pedestrian_env_factory.py) | Compare factory and legacy pedestrian environment creation. | maps/svg_maps/debug_06.svg<br>model/pedestrian/ppo_ped_02.zip<br>output/model_cache/legacy_ppo_run_043/legacy_ppo_run_043.zip | pedestrian, factory | ⚠️ Depends on external models (legacy_ppo_run_043, model/pedestrian/ppo_ped_02.zip). |
...
| [09 Defensive Policy](./advanced/09_defensive_policy.py) | Replay defensive PPO policy with observation adapters. | output/model_cache/legacy_ppo_run_023/legacy_ppo_run_023.zip | policy, ppo | ⚠️ Requires offline PPO checkpoint (legacy_ppo_run_023). |
| [10 Offensive Policy](./advanced/10_offensive_policy.py) | Replay offensive PPO policy in the robot environment. | output/model_cache/legacy_ppo_run_043/legacy_ppo_run_043.zip | policy, ppo | ⚠️ Requires offline PPO checkpoint (legacy_ppo_run_043). |
| [11 Ego Pedestrian Policy](./advanced/11_ego_pedestrian_policy.py) | Run ego pedestrian simulation with recording playback. | maps/svg_maps/debug_06.svg<br>output/model_cache/legacy_ppo_run_043/legacy_ppo_run_043.zip | pedestrian, recording | ⚠️ Requires external PPO checkpoint and long recording playback. |
...
| [32 Demo Adversarial Pedestrian](./advanced/32_demo_adversarial_pedestrian.py) | Demo for running a trained adversarial pedestrian policy. | maps/svg_maps/masterthesis/intersection.svg<br>output/model_cache/legacy_ppo_run_043/legacy_ppo_run_043.zip<br>model/pedestrian/ppo_intersection.zip | pedestrian, policy, debug, ppo | ⚠️ Interactive pygame debug demo with offline checkpoints. |
...
| [42 Offline Checkpoint Cold Load](./advanced/42_offline_checkpoint_cold_load.py) | Load one preserved synthetic Proximal Policy Optimization (PPO) checkpoint for a loader-contract smoke; source/config lineage is unavailable. | uv sync --all-extras | offline, checkpoint, validation | ✅ |
| [43 Reproducible Episode Replay](./advanced/43_reproducible_episode_replay.py) | Compare same-seed episode records and a changed-seed input. | configs/scenarios/single/quickstart_demo.yaml | reproducibility, determinism, episode-record | ✅ |
