...
def _boolean_values(rows: Sequence[Mapping[str, Any]], field: str) -> tuple[list[bool], bool, bool]:
    """Return boolean values plus malformed and missing-field status."""
    values: list[bool] = []
    malformed = False
    missing = False
    for row in rows:
        ...
        if isinstance(value, bool):
            ...
            malformed = True
    return values, malformed, missing
...
def _condition_metrics(
    ...
    *,
    ...
    success_values, malformed_row_success, missing_row_success = _boolean_values(rows, "is_success")
    done_info = _mapping(trace.get("done_info"))
    done_success, malformed_done_success = _optional_boolean(done_info, "success")
    success_present = done_success is not None or (bool(success_values) and not missing_row_success)
    ...
    success = (
        ...
        if (
            ...
            or (missing_row_success and done_success is None)
    ...
    row_terminated_values, malformed_row_terminated, _ = _boolean_values(rows, "terminated")
    ...
    row_truncated_values, malformed_row_truncated, _ = _boolean_values(rows, "truncated")
    ...
    success_reason = (
        ...
        else "each executed row must expose is_success unless trace.done_info.success supplies the outcome"
        if missing_row_success and done_success is None
        else None
    )
    distance_reason = (
