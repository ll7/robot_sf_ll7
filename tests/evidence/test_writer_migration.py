...
class TestWriteJsonMarker:
    ...
    def test_marker_present(self, tmp_path: Path) -> None:
        """Verify generated JSON carries the marker required for evidence review."""
    ...
    def test_byte_deterministic(self, tmp_path: Path) -> None:
        """Verify identical JSON inputs yield stable bytes for reproducible hashes."""
        payload = {"a": 1, "b": [2, 3]}
...
class TestWriteCsvMarker:
    ...
    def test_marker_present(self, tmp_path: Path) -> None:
        """Verify generated CSV starts with the marker required for evidence review."""
        path = tmp_path / "data.csv"
        write_csv(path, [{"col": "val"}])
        assert path.read_text(encoding="utf-8").startswith("# AI-GENERATED NEEDS-REVIEW")

    def test_byte_deterministic(self, tmp_path: Path) -> None:
        """Verify identical CSV rows yield stable bytes for reproducible hashes."""
...
class TestWriteTextMarker:
    ...
    def test_marker_with_issue_ref(self, tmp_path: Path) -> None:
        """Verify text evidence starts with provenance that binds its producing issue."""
    ...
    def test_byte_deterministic_with_date(self, tmp_path: Path) -> None:
        """Verify a fixed provenance date makes repeated text output byte-stable."""
        content = "# Report\n"
    ...
    def test_different_date_produces_different_bytes(self, tmp_path: Path) -> None:
        """Verify provenance-date changes intentionally alter evidence bytes."""
        content = "# Report\n"
        path1 = tmp_path / "r1.md"
        path2 = tmp_path / "r2.md"
        write_text(path1, content, issue_ref="robot_sf#4921", marker_date="2026-07-14")
...
class TestWriteSha256sumsMarker:
    ...
    def test_marker_present(self, tmp_path: Path) -> None:
        """Verify checksum manifests carry the marker required for evidence review."""
    ...
    def test_byte_deterministic(self, tmp_path: Path) -> None:
        """Verify identical bundles yield stable checksum manifests."""
