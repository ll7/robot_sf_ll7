...
def test_proof_binding_records_declared_claim_identity(tmp_path: Path) -> None:
    """Strict proof binding retains campaign, question, and estimand identity."""
    ...
    binding = _build_proof_binding(
        manifest_path,
        manifest,
        expected_campaign_config=None,
    )

    assert binding["campaign_id"] == manifest["campaign"]["id"]
    assert binding["question"] == manifest["answerability"]["question"]["research_question"]
    assert binding["estimand"] == manifest["answerability"]["estimand"]["primary"]
    assert binding["source_manifest"] == manifest_path.relative_to(REPO_ROOT).as_posix()


def test_proof_binding_rejects_external_source_manifest(tmp_path: Path) -> None:
    """A digest of an external file cannot authorize repository admission."""
    manifest = yaml.safe_load(EXAMPLE_MANIFEST.read_text(encoding="utf-8"))

    with pytest.raises(ManifestError, match="source research manifest.*repository"):
        _build_proof_binding(
            Path("/etc/hosts"),
            manifest,
            expected_campaign_config=None,
...
def test_evaluate_research_manifest_answerability_rejects_config_mismatch() -> None:
