...
| `social-jym` JAX social-navigation stack | conceptually adjacent only | `docs/context/issue_729_social_jym_assessment.md` ; `docs/context/issue_792_social_jym_source_harness.md` ; `docs/context/issue_905_social_jym_wrapper_spike.md` ; `docs/context/issue_907_social_jym_sarl_parity.md` | JAX source-harness state/info/action/reset-key contract with SocialNav/LaserNav observations, ORCA/SFM/HSFM human models, and CADRL/SARL/SARL-PPO-style policy surfaces | Adapter-heavy; overlapping holonomic/unicycle action semantics; one-human SARL input parity is proven, but holonomic-to-unicycle projection loses lateral/reverse semantics and trained-policy provenance is missing | Source-harness reproduction, one-step random-action SARL Robot SF wrapper smoke, and controlled one-human input parity now work in isolated/ephemeral dependency environments; still not current Robot SF benchmark support until trained-policy provenance and broader parity are proven |
