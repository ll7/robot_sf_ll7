...
For a bounded inactivity check, use `classify_worker_inactivity` from
`scripts/dev/autopilot_state_snapshot.py` with synthetic observations containing `agent_id`,
`elapsed_seconds`, `wait_status`, `scoped_git`, and `required_artifacts`. Two consecutive
no-progress observations are required before a running worker is reported as stalled. An explicit
productive test/edit status, scoped path change, or required-artifact change resets that streak;
quiet output alone is not a stall signal. The report is diagnostic route evidence and recommends
`interrupt`, `close`, or `parent_fallback` without performing any lifecycle action.
