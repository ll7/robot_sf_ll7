...
def detect_episode_boundaries(states: List[VisualizableSimState]) -> List[int]:
    ...
    for i in range(1, len(states)):
        ...
        if (hasattr(prev_state, 'robot_pose') and prev_state.robot_pose and 
            hasattr(curr_state, 'robot_pose') and curr_state.robot_pose):
        ...
        if not reset_detected and (
            hasattr(prev_state, 'pedestrian_positions') and prev_state.pedestrian_positions is not None and 
            hasattr(curr_state, 'pedestrian_positions') and curr_state.pedestrian_positions is not None and
            len(prev_state.pedestrian_positions) > 0 and len(curr_state.pedestrian_positions) > 0
        ):
            ...
            ped_distance = ((curr_ped_pos[0] - prev_ped_pos[0])**2 + (curr_ped_pos[1] - prev_ped_pos[1])**2)**0.5
...
def main():
    ...
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate converted files by loading them"
    )
    parser.add_argument(
        "--pattern",
        default="*.pkl",
        help="File pattern for directory processing (default: *.pkl)"
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    if input_path.is_file():
        ...
        # Convert directory
        files_converted, total_episodes = convert_directory(
            input_path, output_path, args.pattern
        )
