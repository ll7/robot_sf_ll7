...
def compute_config_digest(config: ResidualSearchConfig) -> str:
    ...
    with the same values compare equal regardless of construction order.
    """
    payload = {
...
# ---------------------------------------------------------------------------
# Candidate evaluation through the non-stateful bound pipeline
# ---------------------------------------------------------------------------
...
def _evaluate_candidate(
    ...
    """Evaluate one candidate through the non-stateful bound pipeline.

    The candidate is placed into a full ``(N, 2)`` proposal with zeros for
    non-targeted rows, then passed through ``clamp_magnitude``,
    ``bound_speed``, and ``bound_heading_change``.  The jerk bound is
    stateful and enforced by ``BoundedResidualAdversary`` after the policy
    returns; walkable-space and separation bounds are likewise enforced by
    the controller.
    ...
    proposal[target_idx] = np.asarray(candidate_2d, dtype=float)
    try:
        bounded = clamp_magnitude(proposal, residual_config.max_residual_accel_mps2)
        bounded = bound_speed(
            bounded,
            velocities,
            max_speeds,
            dt_s,
            residual_config.max_speed_delta_mps,
        )
        per_step_allowance = residual_config.max_heading_change_per_macro_rad / macro_steps
        bounded = bound_heading_change(bounded, velocities, per_step_allowance, dt_s)
        score = float(np.linalg.norm(bounded[target_idx]))
        return score, True
    except (ResidualBoundConflictError, ValueError, TypeError):
        return 0.0, False
...
class FiniteGridSearchPolicy:
    ...
    This policy implements :class:`ResidualAdversaryPolicy` and evaluates a
    finite grid of candidate accelerations through the existing non-stateful
    bound pipeline (``clamp_magnitude``, ``bound_speed``,
    ``bound_heading_change``).  The best candidate per targeted pedestrian is
    selected using the configured diagnostic objective proxy.

    The jerk bound is stateful and enforced by
    :class:`~robot_sf.ped_npc.residual_adversary.BoundedResidualAdversary`
    after this policy returns; this policy does not bypass or weaken it.
    Walkable-space projection and inter-agent separation are likewise enforced
    by the controller.
    ...
    def __init__(
        ...
        self._macro_steps = max(1, round(residual_config.macro_action_dt_s / dt_s))
        self._grid = _build_action_grid(
            search_config.action_min_mps2,
            search_config.action_max_mps2,
            search_config.grid_points_per_dim,
        )
        self._config_digest = compute_config_digest(search_config)
    ...
    def propose_residual(self, observation: ResidualAdversaryObservation) -> np.ndarray:
        ...
        evaluates each through the non-stateful bound pipeline, and selects
        ...
        budget_per_ped = max(1, self._search_config.max_candidates // num_targeted)
        candidates_per_ped = min(grid_size, budget_per_ped)
        ...
        for target_idx in target_indices:
            ...
            for g_idx in range(candidates_per_ped):
