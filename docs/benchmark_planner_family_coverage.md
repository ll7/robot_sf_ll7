...
| `social-jym` JAX social-navigation stack | conceptually adjacent only | `docs/context/issue_729_social_jym_assessment.md` | JAX source-harness state/info/action/reset-key contract with SocialNav/LaserNav observations, ORCA/SFM/HSFM human models, and CADRL/SARL/SARL-PPO-style policy surfaces | Adapter-heavy; overlapping holonomic/unicycle action semantics, but source observations, PRNG keys, reward contracts, and submodule dependencies are not Robot SF planner protocol inputs | Reference and follow-up research candidate only; documented setup failed closed on SSH submodules and missing JAX/submodule packages, so this is not current benchmark support |
