...
from robot_sf.baselines.social_force import SocialForcePlanner, SFPlannerConfig, Observation
...
class TestSocialForceIntegration:
    ...
    def test_minimal_scenario_completion(self):
        ...
        for step in range(max_steps):
            ...
            goal_distance = ((robot_pos[0] - goal_pos[0])**2 + 
                           (robot_pos[1] - goal_pos[1])**2)**0.5
            
            if goal_distance < goal_tolerance:
                break
        
        # Should reach goal within time limit
        final_distance = ((robot_pos[0] - goal_pos[0])**2 + 
                         (robot_pos[1] - goal_pos[1])**2)**0.5
        assert final_distance < goal_tolerance, f"Failed to reach goal, final distance: {final_distance}"
    ...
    def test_pedestrian_avoidance_scenario(self):
        ...
        for step in range(100):
            ...
            ped_distance = ((robot_pos[0] - pedestrian_pos[0])**2 + 
                           (robot_pos[1] - pedestrian_pos[1])**2)**0.5
        ...
        assert min_distance_to_ped > safe_distance * 0.8, f"Too close to pedestrian: {min_distance_to_ped}"
        
        # Should show avoidance behavior (deviating from straight line)
        assert max_distance_to_ped > min_distance_to_ped + 0.5, "No clear avoidance behavior detected"
