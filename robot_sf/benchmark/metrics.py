...
def time_to_goal_norm(data: EpisodeData, horizon: int) -> float:
    ...
    if success_rate(data, horizon=horizon) == 1.0:
        assert data.reached_goal_step is not None  # Success requires a recorded goal step.
...
def clear_tracking_metrics(data: EpisodeData) -> dict[str, Any]:
    ...
    def _float_value(key: str, default: float = float("nan")) -> float:
        """Read numeric field ``key`` from ``raw`` as float, returning ``default`` on failure.

        Returns:
            The numeric field as a float, or ``default``.
        """
        try:
    ...
    def _count_value(key: str) -> float:
        """Read a non-negative count field, coercing non-finite or negative values to ``0.0``.

        Returns:
            The non-negative count value (``0.0`` when invalid).
        """
