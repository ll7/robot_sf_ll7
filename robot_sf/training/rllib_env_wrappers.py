...
class FlattenDictObservationWrapper(ObservationWrapper):
    ...
    feature layout independent of runtime dict ordering. Nested ``spaces.Dict`` subtrees
    are traversed recursively in declaration order.
    """
    ...
    def __init__(self, env: Any, *, keys: Sequence[str] | None = None) -> None:
        ...
        Args:
            ...
            keys: Ordered top-level observation keys to concatenate into one vector.
                When ``None``, all top-level keys from the observation space are flattened.
...
class DriveStateRaysFlattenWrapper(FlattenDictObservationWrapper):
    """Backwards-compatible drive-state/rays flattening wrapper."""
