...
ACTIONLINT_RUNTIME_DIR=""

cleanup_actionlint_runtime() {
  if [[ -n "${ACTIONLINT_RUNTIME_DIR:-}" && -d "$ACTIONLINT_RUNTIME_DIR" ]]; then
    rm -rf -- "$ACTIONLINT_RUNTIME_DIR"
  fi
}
...
bind_actionlint_binary() {
  ...
  local bound_bin="${runtime_dir}/actionlint"
  ...
  if ! candidate_dir="$(cd -P "$candidate_dir" 2>/dev/null && pwd)"; then
    echo "check_github_actions_workflows: cannot resolve ${source} candidate: ${candidate}" >&2
    return 1
  fi
  absolute_candidate="${candidate_dir}/${candidate_name}"
  if [[ ! -f "$absolute_candidate" ]] || [[ ! -x "$absolute_candidate" ]]; then
    echo "check_github_actions_workflows: ${source} candidate is not an executable file: ${absolute_candidate}" >&2
    return 1
  fi
  if ! cp "$absolute_candidate" "$bound_bin" || ! chmod 700 "$bound_bin"; then
    echo "check_github_actions_workflows: could not bind ${source} candidate: ${absolute_candidate}" >&2
    return 1
  fi
  if ! validate_actionlint_binary "$bound_bin" "$platform" "$source"; then
    return 1
  fi
  printf '%s\n' "$bound_bin"
}
...
find_or_install_actionlint() {
  ...
  if [[ -n "${ACTIONLINT_BIN:-}" ]]; then
    local override_bin
    if ! override_bin="$(
      bind_actionlint_binary "$ACTIONLINT_BIN" "$platform" "ACTIONLINT_BIN" "$runtime_dir"
    )"; then
      exit 1
    fi
    echo "$override_bin"
  ...
  if [[ -e "$cached_bin" ]] || [[ -L "$cached_bin" ]]; then
    local bound_cached_bin
    if ! bound_cached_bin="$(
      bind_actionlint_binary "$cached_bin" "$platform" "cached actionlint" "$runtime_dir"
    )"; then
      exit 1
    fi
    echo "$bound_cached_bin"
    return 0
  fi
  ...
  if command -v actionlint >/dev/null 2>&1; then
    ...
    local bound_path_bin
    if ! bound_path_bin="$(
      bind_actionlint_binary "$path_bin" "$platform" "PATH actionlint" "$runtime_dir"
    )"; then
      exit 1
    fi
    echo "$bound_path_bin"
  ...
  local bound_installed_bin
  if ! bound_installed_bin="$(
    bind_actionlint_binary "$cached_bin" "$platform" "installed actionlint" "$runtime_dir"
  )"; then
    rm -rf "$tmpdir"
    exit 1
  fi
  rm -rf "$tmpdir"
  echo "$bound_installed_bin"
}
...
main() {
  ...
  ACTIONLINT_RUNTIME_DIR="$(mktemp -d 2>/dev/null || mktemp -d -t 'actionlint_runtime')"
  ACTIONLINT_RUNTIME_DIR="$(cd -P "$ACTIONLINT_RUNTIME_DIR" && pwd)"
  chmod 700 "$ACTIONLINT_RUNTIME_DIR"
  trap cleanup_actionlint_runtime EXIT

  local actionlint_bin
  actionlint_bin="$(find_or_install_actionlint "$ACTIONLINT_RUNTIME_DIR")"
