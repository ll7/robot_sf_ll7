...
def test_create_map():
    _map = ContinuousOccupancy(10, lambda: None, lambda: None, lambda: np.array([[]]), lambda: np.array([[]]))
...
def test_is_collision_with_obstacle_segment_fully_contained_inside_circle():
    ...
    _map = ContinuousOccupancy(10, lambda: robot_pos, lambda: None, lambda: obstacle_pos, lambda: np.array([[20, 20]]), robot_radius=2)
...
def test_is_collision_with_obstacle_segment_outside_circle():
    ...
    _map = ContinuousOccupancy(10, lambda: robot_pos, lambda: None, lambda: obstacle_pos, lambda: np.array([[20, 20]]), radius)
    assert _map.is_robot_collision
...
def test_is_collision_with_pedestrian():
    ...
    _map = ContinuousOccupancy(40, lambda: robot_pos, lambda: None, lambda: np.array([[]]), lambda: np.array([ped_pos]), robot_radius=2)
    assert _map.is_robot_collision
