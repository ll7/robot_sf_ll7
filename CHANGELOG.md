...
* Added a BR-08 DreamerV3 launcher wrapper (`scripts/training/run_dreamerv3_br08.sh`) plus run-level `run_meta.json`/`resolved_config.json` artifacts so gate/full training runs are easier to submit, audit, and reproduce.
* Added benchmark-aligned BR-08 DreamerV3 configs using flattened `socnav_struct + occupancy_grid`, and extended the Dreamer observation wrapper so RLlib runs can consume the benchmark observation contract without falling back to `drive_state + rays`.
