#!/usr/bin/env python3
"""Delta-only actionable-change monitor for the RobotSF research queue (issue #6819).

The monitor scans the open research surface of the repository (research and
benchmark pull requests, and the open issues that carry research/benchmark
labels or terms), builds a canonical, deterministic set of actionable findings,
and computes a SHA-256 fingerprint over that finding set. It compares the
fingerprint with the marker embedded in the body of the dedicated alert issue
(#6819) and writes a new body only when the fingerprint changes.

Safety contract
---------------
- The monitor is advisory and read-only with respect to every GitHub object
  except its single dedicated target issue. It never merges a pull request,
  never changes research state or labels, never submits or mutates compute,
  never creates issues, and never updates any issue other than #6819.
- An empty scan (no actionable findings) and an unchanged fingerprint are
  no-ops: no write is performed and the run exits ``0``.
- Any GitHub API error aborts the run as a failed/no-write execution: the
  process exits non-zero and no issue body is written.
- The last fingerprint manifest is kept in the body of the dedicated issue and
  the target issue is always re-read (confirming the target number and
  repository) immediately before any body write.

The finding and fingerprint computation is intentionally pure (no I/O) so the
contract is unit-testable offline under ``tests/ci/``.
"""
...
def build_findings(
    ...
    *,
    ...
    The result is pure: the same inputs always yield the same findings, which
    is what makes the fingerprint stable across identical scans.
    """
    findings: list[dict[str, Any]] = []
...
def _gh_json(
    ...
    *,
    ...
    """Run a bounded GitHub CLI JSON read or single-object write."""
    ...
    try:
        ...
        raise MonitorError(f"GitHub API call failed: {exc}") from exc
    if result.returncode != 0:
        detail = (result.stderr or result.stdout).strip()
        raise MonitorError(f"GitHub API call failed for {endpoint}: {detail}")
    if not result.stdout.strip():
        return None
    try:
        return json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        raise MonitorError(f"GitHub returned invalid JSON for {endpoint}: {exc}") from exc
...
def _fetch_paginated(
    ...
    *,
    ...
    """Read a paginated list endpoint with bounded, explicit pages."""
    ...
    for page in range(1, max_pages + 1):
        payload = _gh_json(
            path,
            repo=repo,
            fields={"per_page": str(per_page), "page": str(page)},
        )
        if not isinstance(payload, list):
            raise MonitorError(f"paginated response for {path} is not a list")
...
def _fetch_snapshot(
    ...
    pull_requests = _fetch_paginated(
        f"repos/{repo}/pulls?state=open&sort=updated&direction=desc",
        repo=repo,
        limit=limit,
    )
    issues_payload = _fetch_paginated(
...
def _read_target_issue(repo: str, issue_number: int) -> dict[str, Any]:
    ...
    payload = _gh_json(f"repos/{repo}/issues/{issue_number}", repo=repo)
...
def _update_target_issue(
    ...
    *,
    ...
    The target is re-read immediately before the write and the repository and
    issue number are asserted so the monitor can never write outside its pinned
    target. The recomputed fingerprint is compared against the freshly read
    body marker: when they are equal the write is skipped as a no-op, which
    keeps the delta contract safe against concurrent writers.
    """
    if repo != DEFAULT_REPO or issue_number != DEFAULT_ISSUE:
        raise MonitorError(
            f"refusing to write outside the pinned target: got {repo}#{issue_number}"
        )
    current = _read_target_issue(repo, issue_number)
    if extract_previous_fingerprint(str(current.get("body") or "")) == expected_fingerprint:
        return
...
def run_monitor(
    *,
    ...
    | None = None,
    read_target_issue: Callable[..., dict[str, Any]] | None = None,
    update_target_issue: Callable[..., None] | None = None,
) -> dict[str, Any]:
    """Run one delta-only scan and update only the dedicated issue when needed.

    ``fetch_snapshot``, ``read_target_issue``, and ``update_target_issue`` are
    injectable seams so the pure contract (empty scan no-op, unchanged
    fingerprint no-op, changed fingerprint writes, API errors fail closed) is
    testable without a live ``gh`` executable.
    """
    scanned_at = now or datetime.now(UTC)
    if fetch_snapshot is None:
        fetch_snapshot = _fetch_snapshot
    if read_target_issue is None:
        read_target_issue = _read_target_issue
    if update_target_issue is None:
        update_target_issue = _update_target_issue

    pull_requests, issues, checks = fetch_snapshot(repo, limit)
    ...
    previous = extract_previous_fingerprint(
        str(read_target_issue(repo, issue_number).get("body") or "")
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"actionable-change monitor failed closed: {exc}", file=sys.stderr)
        return 1
