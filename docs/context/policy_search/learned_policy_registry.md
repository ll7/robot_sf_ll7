...
| `tentabot_value_scorer_v0` | `external_learned_policy` | `staged` | `smoke_and_nominal_diagnostic` | `smoke_only` | Clean-room Robot SF scorer spike is executable and smoke-passes on `planner_sanity_simple`; #1826 lowers nominal-sanity collision and near-miss rates, while #1832 confirms attempted progress-recovery retunes either leave outcomes unchanged or trade fewer low-progress timeouts for more collisions. The row remains `revise`. Upstream Tentabot remains source-side only. |
| `tentabot_value_scorer_v1_static_gated` | `external_learned_policy` | `staged` | `smoke_and_nominal_diagnostic` | `not_benchmark_evidence` | Clean-room v1 keeps the v0 scorer/lattice and adds a deterministic static-safety demotion tier for low-clearance candidates. It smoke-passes, but nominal sanity stays at 4/18 success while collisions rise to 2/18 and near misses to 4/18, so the static-gate lane is `revise` / stop rather than promotion evidence. |
| `tentabot_value_scorer_v2_route_arc` | `external_learned_policy` | `staged` | `smoke_and_nominal_diagnostic` | `not_benchmark_evidence` | Clean-room v2 keeps the v1 static-safety gate and adds a route-arc progress value term for every accepted candidate when route geometry is available. It smoke-passes, but nominal sanity stays at 4/18 success while collisions remain 2/18, near misses rise to 5/18, and low-progress timeouts fall only to 8/18; classify as `revise`, not promotion evidence. |
| `tentabot_value_scorer_v3_trace_recovery` | `external_learned_policy` | `staged` | `smoke_and_nominal_diagnostic` | `not_benchmark_evidence` | Clean-room v3 keeps the v1 static-safety gate and adds trace-level recovery that only selects already accepted route-recovery candidates under recent route regression or combined route/goal stall. It is executable, but the smoke route times out and nominal sanity stays at 4/18 success with 2/18 static collisions, 3/18 near misses, and 9/18 low-progress timeouts; classify as `revise` / stop. |
