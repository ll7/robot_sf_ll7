...
| `scripts/dev/snapshot_pr_queue.py` | `gh pr view` / `gh pr list --json ...`, REST `pulls/<n>`, plus `commits/main` | `number`, `title`, `state`, `isDraft`, `labels`, `url`, `headRefName`, `headRefOid`, `baseRefName`, `baseRefOid`, `mergeable`, `statusCheckRollup`, `reviews`, `comments`, current `main` SHA | Active PR triage, review summaries, comment excerpts, labels, CI status, stale-head and stale-base checks | Required but body-bearing | Keep the bounded field set for the `pr_queue_snapshot.v2` contract. Review/comment bodies are truncated before stdout; base freshness records the exact comparison and fail-closed reason. REST base lookup preserves compatibility with older gh versions that lack `baseRefOid`. |
