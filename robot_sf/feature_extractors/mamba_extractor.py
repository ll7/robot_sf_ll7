...
def _init_classes() -> dict[str, Any]:  # noqa: C901
    ...
    class MambaFeatureExtractor(BaseFeaturesExtractor):
        ...
        def __init__(  # noqa: PLR0913
            ...
            drive_hidden_dims = tuple(
                drive_hidden_dims if drive_hidden_dims is not None else (32, 16)
            )
            ...
            self.sequence_layers, self.backend_name, self.backend_exact = (
                self._build_sequence_layers(
                    backend=backend,
                    d_model=d_model,
                    d_state=d_state,
                    d_conv=d_conv,
    ...
    return {
        "_TorchSSMLiteBlock": _TorchSSMLiteBlock,
        "MambaFeatureExtractor": MambaFeatureExtractor,
    }


_cache: dict[str, Any] | None = None
_LAZY_NAMES = {"_TorchSSMLiteBlock", "MambaFeatureExtractor"}


def __getattr__(name: str) -> Any:
    if name == "MambaFeatureExtractorConfig":
        return MambaFeatureExtractorConfig
