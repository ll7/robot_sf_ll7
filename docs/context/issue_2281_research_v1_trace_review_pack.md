...
| --- | --- | --- | --- | --- |
| `ammv_head_on_corridor_mechanism_activation` | telemetry parity annotated | Direct AMMV force term activated in a mechanism probe; Issue #2405 proved selected default/AMMV aggregate rows can export to loader-valid `simulation_trace_export.v1`, Issue #2428 published one durable rendered default/AMMV trace-panel pair, Issue #2430 found that selected pair is frame-identical, and Issue #2432 found the local seed `111..113` slice is also frame-identical. | `gather_more_evidence` | The proof is diagnostic-only and validates telemetry/rendering, but the current head-on-corridor adapter-mode slice is not a behavioral-difference source. A different scenario/family, direct planner-mode trace source, or extra instrumentation is needed. |
...
missing-input blocker, and one AMMV case whose single-row trace-export path now has a first durable
diagnostic rendered panel pair plus telemetry-parity annotation and seed-slice selection decisions,
but still lacks a behavioral-difference trace case.

## Synthesis Table

| Mechanism | Source issue | Evidence tier | Config | Seeds | Artifacts | Metrics | Verdict | Caveats |
| --- | ---: | --- | --- | --- | --- | --- | --- | --- |
| AMMV Social Force direct activation | #2168 / #2269 / #2309 / #2405 / #2428 / #2430 / #2432 | diagnostic mechanism probe plus trace-export, rendered-panel, frame-parity annotation, and seed-slice selection proof | `classic_head_on_corridor_low` direct probe and #2168 benchmark rerun commands | `111`, `112`, `113` | `docs/context/evidence/issue_2168_ammv_social_force_pair_2026-06-03/summary.json`; `docs/context/evidence/issue_2309_amv_trace_export_probe_2026-06-05/summary.json`; `docs/context/evidence/issue_2405_amv_step_export_2026-06-06/summary.json`; `docs/context/evidence/issue_2428_mechanism_trace_panels_2026-06-06/summary.json`; `docs/context/evidence/issue_2430_ammv_trace_annotation_2026-06-06/summary.json`; `docs/context/evidence/issue_2432_ammv_trace_selection_2026-06-06/summary.json` | AMMV force magnitude and intrusion count; selected default/AMMV aggregate rows preserve step frames, export with `ammv.pedestrian_force_vectors`, render as a first durable diagnostic panel pair, and compare frame-identical across 100 frames for seeds `111..113` | `telemetry_parity_not_behavior_delta` | Diagnostic-only telemetry proof; the current head-on-corridor adapter-mode slice is not a behavioral-difference source, so the next step must leave this slice or add instrumentation. |
...
- select a different AMMV scenario/family or direct planner-mode mechanism trace if the lane needs
  behavioral-difference evidence, because Issues #2430 and #2432 found the current
  head-on-corridor adapter-mode seed slice is frame-identical, or
