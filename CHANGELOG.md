...
* Added an all-planners paper-facing benchmark release surface for tag `0.0.2` with `configs/benchmarks/paper_experiment_matrix_all_planners_v1.yaml`, its release manifest under `configs/benchmarks/releases/`, comparability-map coverage for `socnav_bench`, and a durable execution log in `docs/context/benchmark_release_0_0_2_2026-04-13.md`.
* Camera-ready planner rows now expose `most_likely_failure_reason` and campaign reports include a dedicated “Failed Planners And Likely Reasons” section; the all-planners paper-facing config for tag `0.0.2` now uses `stop_on_failure: false` so one planner failure is explicit without aborting the full campaign.* Added first scoped benchmark release v0.0.2 (7-planner release excluding socnav_bench due to missing external SocNavBench dataset dependency): new campaign config `paper_experiment_matrix_7planners_v1.yaml`, release manifest `paper_experiment_matrix_7planners_v1_release_v0_0_2_scoped.yaml`, and durable rationale note in `docs/context/benchmark_release_0_0_2_scoped_rationale.md` with full historical record of blockage and future-path guidance for upgrading to full 8-planner release once assets are staged.* Added Project #5 task prioritization support: a documented benchmark-oriented scoring model, a `project_priority_score.py` sync helper, and a GitHub Actions workflow for manual/scheduled/issue-event score synchronization into a numeric `Priority Score` field.
