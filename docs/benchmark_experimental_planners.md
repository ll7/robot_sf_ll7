...
| `brne` | `docs/context/issue_6464_brne_corridor_diagnostic.md`, `docs/context/issue_6934_brne_action_scaling.md`, `docs/context/issue_5318_brne_integration_decision.md`, `docs/context/issue_5311_brne_source_smoke.md` | Source-aligned action aggregation is now validated: mean-normalized upstream weights are combined with a sample mean, removing the effective-sample-count scale error. The exact native corridor diagnostic remains 0/3 goal-reaching with no corridor violations or fallback/degraded rows; this is diagnostic evidence only, not a benchmark result. | Diagnose progress/goal-reaching after the contract correction before proposing any broader or ranked campaign. | [#6934](https://github.com/ll7/robot_sf_ll7/issues/6934) resolves action scaling; [#6938](https://github.com/ll7/robot_sf_ll7/issues/6938) owns the residual progress diagnosis. Retain fail-closed fallback/runtime provenance, native-core-via-adapter labeling, and corridor-only scope. |
