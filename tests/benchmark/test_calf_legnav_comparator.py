...
def test_partial_success_flags_without_trace_outcome_are_unavailable() -> None:
    """A partial row-success field cannot produce a success metric from surviving rows."""
    perfect = _trace("ideal_state", [2.0, 2.0, 2.0])
    sensor = _trace("perception_limited", [2.0, 2.0, 2.0])
    for trace in (perfect, sensor):
        trace["done_info"].pop("success")
        trace["steps"][1].pop("is_success")
    ...
    assert success["reason"] == (
        "each executed row must expose is_success unless "
        "trace.done_info.success supplies the outcome"
    )
    assert report["conditions"]["perfect_perception"]["status"] == "blocked"
    assert report["status"] == "blocked"


def test_trace_success_outcome_supplies_missing_row_success_flags() -> None:
    """A trace-level success verdict may complete rows that omit is_success."""
    perfect = _trace("ideal_state", [2.0, 2.0, 2.0])
    sensor = _trace("perception_limited", [2.0, 2.0, 2.0])
    perfect["steps"][1].pop("is_success")
