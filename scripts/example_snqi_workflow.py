    ...
    for algo in algorithms:
        for density in densities:
            for seed in range(5):  # 5 episodes per algo-density combination
                ...
                time_to_goal_norm = min(1.0, max(0.1, 
                    np.random.normal(1.0 - time_efficiency, 0.2)))
                
                min_distance = max(0.1, np.random.normal(1.5 / density_factor, 0.3))
                path_efficiency = max(0.3, min(1.0, np.random.normal(0.8, 0.1)))
                
                comfort_exposure = max(0.0, min(1.0, 
                    np.random.normal(collision_rate * 2, 0.1)))
...
def demonstrate_weight_recomputation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"   Saved data to temporary files")
        ...
        try:
            ...
            print(f"   - Most stable normalization: median_p95")
            print(f"   - Weight sensitivity varies by component")
            ...
            print(f"   python scripts/recompute_snqi_weights.py --episodes {episodes_file} --baseline {baseline_file} --compare-strategies --output results.json")
