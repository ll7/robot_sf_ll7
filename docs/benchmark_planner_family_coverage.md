...
| `social-jym` JAX social-navigation stack | conceptually adjacent only | `docs/context/issue_729_social_jym_assessment.md` ; `docs/context/issue_792_social_jym_source_harness.md` ; `docs/context/issue_905_social_jym_wrapper_spike.md` | JAX source-harness state/info/action/reset-key contract with SocialNav/LaserNav observations, ORCA/SFM/HSFM human models, and CADRL/SARL/SARL-PPO-style policy surfaces | Adapter-heavy; overlapping holonomic/unicycle action semantics, but source observations, PRNG keys, reward contracts, and submodule dependencies are not Robot SF planner protocol inputs | Source-harness reproduction and a one-step random-action SARL Robot SF wrapper smoke now work in isolated/ephemeral dependency environments; still not current Robot SF benchmark support until parity and trained-policy provenance are proven |
