...
- Eval-side parity follow-up now includes targeted runtime checks in
  `tests/training/test_train_dreamerv3_rllib_runtime.py` for evaluation-matrix selection and
  base-env contract parity.
- Encoder/decoder or world-model pretraining from rollouts generated by PPO, ORCA, or other
  ...
  model outside RLlib. See `docs/context/issue_782_dreamerv3_pretraining_design.md`.
- The multimodal Dict-observation path now fails closed on the current Ray 2.53.0 DreamerV3
  catalog; see `docs/context/issue_789_dreamer_multimodal_encoder.md`.
