...
uv run python -m robot_sf.benchmark.cli run \
  ...
  --output output/benchmarks/quickstart/my_experiment_episodes.jsonl \
  --workers 4 \
...
uv run python -m robot_sf.benchmark.cli baseline \
  ...
  --output output/benchmarks/quickstart/my_experiment_baseline.jsonl \
  --workers 4
```
...
for ALGO in "social_force" "ppo" "random"; do
  uv run python -m robot_sf.benchmark.cli run \
    ...
    --output output/benchmarks/quickstart/episodes_${ALGO}.jsonl \
    --algo $ALGO \
    --workers 4 \
    --resume
done
...
uv run python -m robot_sf.benchmark.cli snqi-recompute \
  --episodes output/benchmarks/quickstart/my_experiment_episodes.jsonl \
  --output output/benchmarks/quickstart/recomputed_episodes.jsonl \
  --weights configs/snqi_weights/custom_weights.json

# Weight sensitivity analysis
uv run python -m robot_sf.benchmark.cli snqi-weight-ablation \
  --episodes output/benchmarks/quickstart/my_experiment_episodes.jsonl \
  --output output/benchmarks/quickstart/weight_ablation/ \
...
uv run python -m robot_sf.benchmark.cli run \
  ...
  --output output/benchmarks/quickstart/parameter_sweep_episodes.jsonl \
  --workers 8 \
  --resume \
  --batch-size 50
```
...
uv run python -m robot_sf.benchmark.cli figures \
  --episodes output/benchmarks/quickstart/my_experiment_episodes.jsonl \
  --output-dir output/benchmarks/quickstart/figures/ \
...
uv run python -m robot_sf.benchmark.cli figure-distribution \
  --episodes output/benchmarks/quickstart/my_experiment_episodes.jsonl \
  --output output/benchmarks/quickstart/figures/distribution.png \
  --metric "metrics.snqi" \
  --group-by "scenario_params.algo"

uv run python -m robot_sf.benchmark.cli figure-pareto \
  --episodes output/benchmarks/quickstart/my_experiment_episodes.jsonl \
  --output output/benchmarks/quickstart/figures/pareto.png \
...
uv run python -m robot_sf.benchmark.cli extract-trajectories \
  --episodes output/benchmarks/quickstart/my_experiment_episodes.jsonl \
  --output output/benchmarks/quickstart/trajectories/ \
  --scenario-filter "dense_navigation" \
  --format pickle

# Generate trajectory animations  
uv run python examples/trajectory_demo.py \
  --trajectory-file output/benchmarks/quickstart/trajectories/dense_navigation_42.pkl \
  --output output/benchmarks/quickstart/animations/dense_navigation_42.mp4
```
...
uv run python -m robot_sf.benchmark.cli validate-episodes \
  --episodes output/benchmarks/quickstart/problematic_episodes.jsonl \
  --fix-corruption
```
...
uv run python -m robot_sf.benchmark.cli run \
  ...
  --output output/benchmarks/quickstart/policy_comparison.jsonl \
  --workers 4
...
uv run python -m robot_sf.benchmark.cli figure-pareto \
  --episodes output/benchmarks/quickstart/policy_comparison.jsonl \
  --output output/benchmarks/quickstart/policy_pareto.png \
  --x-metric "metrics.time_to_goal" \
  --y-metric "metrics.snqi"
...
uv run python -m robot_sf.benchmark.cli run \
  ...
  --output output/benchmarks/quickstart/comprehensive_episodes.jsonl \
  --workers 8 \
  --resume
...
uv run python -m robot_sf.benchmark.cli figures \
  ...
  --output-dir output/benchmarks/quickstart/publication_figures/ \
...
uv run python -m robot_sf.benchmark.cli table \
  --summary output/benchmarks/quickstart/comprehensive_summary.json \
  --output output/benchmarks/quickstart/publication_figures/results_table.tex \
  --format latex \
  --precision 3
...
uv run python -m robot_sf.benchmark.cli run \
  ...
  --output output/benchmarks/quickstart/snqi_base_episodes.jsonl \
  --workers 4
...
uv run python -m robot_sf.benchmark.cli analyze-weight-sensitivity \
  --ablation-dir output/benchmarks/quickstart/weight_ablation/ \
  --output output/benchmarks/quickstart/weight_sensitivity_report.json \
  --plot output/benchmarks/quickstart/weight_sensitivity.png

# 4. Generate ranking stability analysis (5 minutes)  
uv run python examples/benchmarks/snqi_full_flow.py \
  --episodes output/benchmarks/quickstart/snqi_base_episodes.jsonl \
  --ablation-results output/benchmarks/quickstart/weight_ablation/ \
  --output output/benchmarks/quickstart/ranking_stability.png

echo "✓ Weight sensitivity analysis complete. See output/benchmarks/quickstart/weight_sensitivity_report.json"
```
...
robot_sf_bench baseline \
  --episodes output/benchmarks/quickstart/episodes.jsonl \
  --output output/benchmarks/quickstart/baseline_stats.json

robot_sf_snqi recompute \
  --baseline-stats output/benchmarks/quickstart/baseline_stats.json \
  --out output/benchmarks/quickstart/snqi_weights_v1.json
```

## 4. Aggregate Metrics with Confidence Intervals
```
robot_sf_bench aggregate \
  --in output/benchmarks/quickstart/episodes.jsonl \
  --out output/benchmarks/quickstart/summary_ci.json \
...
uv run python scripts/generate_figures.py \
  --episodes output/benchmarks/quickstart/episodes.jsonl \
  --table-summary output/benchmarks/quickstart/summary_ci.json \
  --table-include-ci --table-tex \
  --out-dir output/benchmarks/quickstart/figures/episodes_run_v1
```
...
```
robot_sf_snqi ablation --episodes output/benchmarks/quickstart/episodes.jsonl --summary-out output/benchmarks/quickstart/snqi_ablation.json
```
...
run_batch(scenarios, "output/benchmarks/quickstart/episodes.jsonl", workers=4, resume=True)
