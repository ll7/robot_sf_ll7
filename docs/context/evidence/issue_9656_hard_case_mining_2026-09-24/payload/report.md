...
- An earlier provenance refresh used local commit `0a5f73283b98279900797b75d20adf6d4676086b` as its matching snapshot. Exact review later established that commit is available only from a local backup ref and cannot be retrieved from GitHub. The current refresh no longer relies on it; all three recorded source-file hashes match reachable snapshot `120c870...`.
