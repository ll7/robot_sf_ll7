...
def test_build_policy_prediction_mpc_registers_adapter() -> None:
    ...
    assert adapter.prediction_config.predictor_backend == "constant_velocity"
...
def test_build_policy_prediction_mpc_alias_registers_same_adapter() -> None:
    """Legacy alias should route through the canonical prediction-MPC adapter."""
    policy, meta = _build_policy(
...
def test_run_map_episode_excludes_live_foresight_fallback_from_evidence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Map-runner records must carry a live foresight fallback through aggregation."""

    class _DummySim:
        """Simulator stub for the map-runner provenance path."""
    ...
    class _DummyEnv:
        """Environment stub which executes one planner step."""

        def __init__(self, map_def: MapDefinition) -> None:
            self.simulator = _DummySim(map_def)

        def reset(self, seed: int | None = None):
            """Return a minimal map-runner observation."""
            del seed
            return {
                "robot": {"position": [0.0, 0.0], "heading": [0.0]},
                "goal": {"current": [1.0, 1.0]},
        ...
        def step(self, action):
            """Terminate after the policy has materialized its live diagnostics."""
            del action
            return (
                {
                    "robot": {"position": [0.0, 0.0], "heading": [0.0]},
        ...
        def close(self) -> None:
            """Provide the lifecycle hook expected by the episode runner."""

    foresight = {
        "foresight_prediction": {
            "requested_model_id": "unavailable-test-model",
            "requested_checkpoint_sha256": None,
            "load_status": "failed",
            "effective_prediction_mode": "constant_velocity",
            "fallback_used": True,
            "fallback_reason": "predictive_model_load_failed",
            "load_error": "FileNotFoundError: unavailable-test-model",
        }
    }

    def _build_policy_stub(*args, **kwargs):
        """Return a policy with the live PPO/adapter runtime diagnostics hook."""
        del args, kwargs

        def _policy(obs: dict[str, object]) -> tuple[float, float]:
            """Produce a deterministic command after the modeled fallback."""
            del obs
            return 0.0, 0.0
    ...
    record = _run_map_episode(
        {"name": "foresight-fallback", "simulation_config": {"max_episode_steps": 1}},
        seed=1,
        horizon=1,
        dt=0.1,
        record_forces=False,
    ...
    summary = compute_aggregates(
        [
            record,
            {
                ...
                "algo": "prediction_planner",
                "metrics": {"success": 1.0},
                "algorithm_metadata": {"foresight_prediction": {"evidence_eligible": True}},
            },
        ],
        group_by="algo",
    )
    ...
    record = _run_map_job_worker(
        (
            ...
            7,
            {
                ...
                "paired_wrapper_off_record": {"episode_id": "wrapper-off-counterpart"},
                "record_simulation_step_trace": True,
            },
        )
    )
