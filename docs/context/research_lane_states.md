...
| --- | --- | --- | --- | --- | --- |
| Topology selection scoring | #2660 | `revise` | [issue_2660_topology_successor_gate.md](issue_2660_topology_successor_gate.md) | Implement a progress-gated primary-route reselection successor before any further topology smoke; repeated `primary_route` selections should stay penalized only when eligible near-parity alternatives exist and recent primary-route progress is below a predeclared threshold. | Stop treating the reuse-penalty gate as benchmark improvement evidence; do not rerun `topology_guided_hybrid_rule_v0_reuse_penalty` unchanged on the same slice. |
