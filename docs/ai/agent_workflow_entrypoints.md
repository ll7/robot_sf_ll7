...
- **Explicit issue relationships**: Issue and PR workflows read [`issue_relationships.md`](../context/issue_relationships.md), keep canonical `## Relationships` / `## Issue Relationship Mirror` blocks, and set native links only from the owning writable worktree. Review-only worktrees verify fresh state but never infer or write graph edges. Direct CLI relationship operations require `gh >= 2.100.0`; older or unsupported CLIs use the documented REST/UI fallback and fail closed.
