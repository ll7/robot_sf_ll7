...
| PPO model registry | `model/registry.yaml` | partial | Has W&B run/artifact pointer, but `commit: null` and a note points at local `output/model_cache/.../best_summary.json`. |
...
and include the required SNQI diagnostics payload when promoting any compact evidence bundle as
paper support.
