...
* **Issue #6960 per-phenomenon replay videos for the issue #5149 emergence campaign.** Added
  ...
  visualization of existing smoke-tier evidence; no new quantitative claims.
...
* **issue #4908 diagnostic CLI `--help` omits the canonical `uv run` invocation.** The seven
  ...
  to guess the invocation (the bare `python scripts/tools/<name>.py ...` path still dies with <!-- active-docs-check: allow -->
...
* **issue #4873 user-facing docs drift audit — corrected stale claims against current code (docs-only).**
  ...
  `output/benchmarks/` → `output/results/` and the `tests/test_gym_env.py` → `tests/test_gymnasium_env_contracts.py` <!-- active-docs-check: allow -->
