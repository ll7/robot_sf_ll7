...
| `tentabot_value_scorer_v0` | `external_learned_policy` | `staged` | `smoke_and_nominal_diagnostic` | `smoke_only` | Clean-room Robot SF scorer spike is executable and smoke-passes on `planner_sanity_simple`, but the 2026-05-31 nominal-sanity report is `revise` with low success and elevated collision/near-miss rates; follow-up #1826 tracks scorer repair. Upstream Tentabot remains source-side only. |
