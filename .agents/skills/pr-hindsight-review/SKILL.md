...
- `native_relationship_state`: the linked issue's current Parent/Blocked by/Blocking state under
  `docs/context/issue_relationships.md`, and whether it fits the PR's linked-issue scope.
...
3. Inspect linked issue state, including native Parent/Blocked by/Blocking links, only as needed to
   decide full, partial, duplicate, or successor disposition.
...
- Do not infer relationship edges from branch names, commit messages, or incidental issue mentions;
  report stale native-state evidence when the graph matters to the disposition.
