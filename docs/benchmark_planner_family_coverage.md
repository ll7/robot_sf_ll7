...
| CrowdNav_HEIGHT | conceptually adjacent only | `docs/context/issue_742_awesome_robot_social_navigation_mining.md` , `docs/context/issue_629_planner_zoo_research_prompt.md` | CrowdNav-lineage heterogeneous interaction graph transformer with upstream graph/state construction and source-harness checkpoint expectations | Adapter-heavy and runtime-legacy; likely requires side-environment or model-only inference wrapping before any Robot SF `unicycle_vw` claim | External learned-family anchor only; keep as assessment/prototype scope until source-harness validation exists |
