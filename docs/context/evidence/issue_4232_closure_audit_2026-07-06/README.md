...
| Submit a private Slurm benchmark campaign only if justified by diagnostics. | Not met in authorized scope | The live issue comment after PR #4626 states the remaining work is the actual benchmark-strength campaign under a later authorized claim-promotion task. | This run has `compute_submit_authorized=false`; no Slurm/GPU/campaign submission is authorized. |
