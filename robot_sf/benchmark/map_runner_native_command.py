...
class _NoProgressDeadlockDetector:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the stall detector with a window size and progress threshold."""
        if window_steps < 1:
...
class NativeCommandPlanner:
    ...
    def _start_process(self) -> None:
        """Launch the persistent subprocess, terminating any existing one first."""
        self._stop_process()
        try:
    ...
    def _stop_process(self) -> None:
        """Terminate and clear the persistent subprocess and its stdout buffer."""
    ...
    def _plan_per_episode(
        ...
        """Return the native command output from a fresh per-episode subprocess invocation."""
        try:
            proc = subprocess.run(
                self._spec.command,
    ...
    def _plan_persistent(
        ...
        """Return the native command output from the persistent subprocess over stdin/stdout."""
...
def build_native_command_policy(  # noqa: PLR0913
    ...
    *,
    ...
    def _bind_env(env: Any) -> None:
        """Bind the live environment's static geometry to the native planner and metadata."""
