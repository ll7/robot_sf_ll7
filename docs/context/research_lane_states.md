...
| --- | --- | --- | --- | --- | --- |
| Topology selection scoring | #2706 | `stop` | [issue_2706_topology_lane_synthesis.md](issue_2706_topology_lane_synthesis.md) | Open no further same-family selector tie-break variant on the canonical h160 double-bottleneck slice; any renewed topology issue must name a different mechanism and a metric beyond primary-route selection counts. | Stop unchanged reruns of `topology_guided_hybrid_rule_v0`, `topology_guided_hybrid_rule_v0_reuse_penalty`, or `topology_guided_hybrid_rule_v0_progress_gated_reselection` as research-progress evidence on the canonical slice. |
