...
| `research-v1.amv.benchmark_matrix` | A frozen AMV social-navigation matrix can support reproducible planner comparison work. | Tracked config/protocol note, scenario families, planner rows, seed policy, artifact policy, fail-closed row semantics, and config validation. | `proposal` | Issue [#2155](https://github.com/ll7/robot_sf_ll7/issues/2155) defines the freeze task; no matrix run is implied by this note. | Freeze the matrix in #2155 before running research-v1 comparisons. |
