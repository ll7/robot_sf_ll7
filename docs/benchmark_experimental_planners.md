...
To inspect registered planners, baselines, required extras/artifacts, and readiness tiers without starting an expensive simulation run or importing heavy dependencies:
...
The JSON payload keeps benchmark-contract availability separate from metadata completeness.
`availability_status: unknown` means discovery did not probe local optional dependencies; it is
not a claim that the planner is runnable on the current machine. A `not_available` value is a
declared benchmark/readiness exclusion, such as the stochastic `random` diagnostic reference.
Missing compatibility metadata remains explicit as `unknown` (or a source-declared
`not_declared` marker), and `metadata_completeness` reports that omission rather than filling it
from a planner class or runtime guess. When no readiness-matrix row is declared, discovery uses
`readiness_status: not_declared` and a JSON `null` success-evidence value rather than guessing.
...
| `brne` | `docs/context/issue_6464_brne_corridor_diagnostic.md`, `docs/context/issue_6934_brne_action_scaling.md`, `docs/context/issue_6938_brne_progress_mechanism.md`, `docs/context/issue_6944_brne_candidate_transition.md`, `docs/context/issue_5318_brne_integration_decision.md`, `docs/context/issue_5311_brne_source_smoke.md` | Source-aligned action aggregation is validated: mean-normalized upstream weights are combined with a sample mean, removing the effective-sample-count scale error. The exact native corridor diagnostic remains 0/3 goal-reaching with no corridor violations or fallback/degraded rows. Candidate telemetry shows the candidate distribution itself shifts from `v≈0.40` to `v≈0.04 m/s` while robot weights remain uniform and no clipping occurs; parity telemetry additionally shows a straight-constant nominal command and `2/0/2` observed/within-upstream-radius/passed pedestrians. This narrows weighting/normalization and confirms a controller-parity lead, but remains diagnostic evidence only, not a benchmark result. | Explain or falsify the remaining upstream candidate-generation/activation, planner-state, or control-horizon causes before proposing any broader or ranked campaign. | [#6934](https://github.com/ll7/robot_sf_ll7/issues/6934) resolves action scaling; [#6938](https://github.com/ll7/robot_sf_ll7/issues/6938) records the progress signal; [#6944](https://github.com/ll7/robot_sf_ll7/issues/6944) narrows weighting/normalization and confirms a parity lead without changing runtime behavior. Retain fail-closed fallback/runtime provenance, native-core-via-adapter labeling, and corridor-only scope. |
...
| BRNE | `MurpheyLab/brne` @ `633a5cd` (GPL-3.0, IJRR 2024) | BORDERLINE (conditional go) — source-side smoke PASSES: the upstream pure-numpy/numba core (`brne_nav/brne_py/brne_py/brne.py`) runs end-to-end on the redistributable path (numpy+scipy+numba; no ROS2/torch/C++). Native unicycle `(v, ω)` output needs no projection; GP-sampled humans are crowd-model-agnostic. | ~9 / 19 / 39 / 67 / 115 ms steady for 1 / 2 / 3 / 5 / 7 pedestrians (2–8 agents) — under the 100 ms budget up to ~6 agents, ~1× at the 8-agent default cap; ~linear in agent count; one-time JIT compile ~2.4 s amortized per episode. | (1) No native static-obstacle geometry — corridor bounds only (`corridor_y_min/max`); Robot SF static maps would need a labeled cost extension or corridor-class scenario scoping. (2) Robot heading `θ` and pedestrian velocities are required (privileged-level or finite-differenced). (3) Dense crowds within the activation gate could exceed budget — tunable via `num_samples`. GPL-3.0 keeps it local-only (not vendored). | `docs/context/issue_5311_brne_source_smoke.md` (issue #5311) |
