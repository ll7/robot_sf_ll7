...
| Oracle trace artifact access | PR #2556 finalized trace manifests/checksums for the Issue #2441 oracle-imitation runs, and [issue_2620_oracle_artifact_access.md](issue_2620_oracle_artifact_access.md) confirms the split/checksum evidence is intact but raw trace retrieval is blocked: durable pointers are missing and the recorded local artifact paths are absent on the current machine. | [Issue #2620](https://github.com/ll7/robot_sf_ll7/issues/2620) records `artifact_retrieval_blocked`; the next useful action is recovery/promotion from a Slurm-capable or artifact-capable host, or a deliberate rerun that publishes concrete URIs. | [Issue #2569](https://github.com/ll7/robot_sf_ll7/issues/2569) keeps oracle-imitation training gated until durable artifact access is available. |
...
- Oracle trace evidence: [evidence/issue_2441_oracle_imitation_traces_2026-06-06/README.md](evidence/issue_2441_oracle_imitation_traces_2026-06-06/README.md) and [issue_2620_oracle_artifact_access.md](issue_2620_oracle_artifact_access.md)
