...
class MapDefinitionPool:
    ...
    def _load_map_definitions_from_folder(self, maps_folder: str) -> dict[str, MapDefinition]:
        ...
        every robot route has a corresponding reversed counterpart, matching the
        invariant that ``serialize_map`` enforces for legacy JSON maps.  SVG maps
        that were authored with explicit reversed routes (e.g. those generated by
        ``scripts/dev/json_to_svg_map.py``) are not affected because the
        normalisation checks for duplicates before inserting.
        ...
        for name in map_names:
            ...
            if map_def is None:
                # convert_map already logs the failure reason internally.
                continue

            map_def = _ensure_symmetric_robot_routes(map_def)
...
def _ensure_symmetric_robot_routes(map_def: MapDefinition) -> MapDefinition:
    """Return *map_def* with a reversed counterpart added for every robot route that lacks one.

    ``serialize_map`` always doubles robot routes (forward + reversed).  SVG
    maps authored without reversed routes would otherwise produce half as many
    routes, causing robots to be unable to travel back along the same path.
    This function brings SVG-loaded maps in line with that invariant without
    duplicating routes that were already included in the SVG.

    Args:
        map_def: Map definition as returned by ``convert_map``.

    Returns:
        MapDefinition: A copy of *map_def* with symmetric robot routes.
    """
    existing_pairs = {(r.spawn_id, r.goal_id) for r in map_def.robot_routes}
    missing = [
        _reverse_route(r)
        for r in map_def.robot_routes
        if (r.goal_id, r.spawn_id) not in existing_pairs
    ]
    if not missing:
        return map_def
    from dataclasses import replace  # noqa: PLC0415

    return replace(map_def, robot_routes=map_def.robot_routes + missing)
