...
def test_main_only_drift_remains_integrable_when_feature_head_is_unchanged() -> None:
    """A stable captured feature head must not become an undeclared stack when main advances."""
    baseline = _snapshot(
...
def test_main_drift_with_post_capture_head_movement_remains_blocked() -> None:
    """A new local commit after capture must not bypass undeclared-stack protection."""
    baseline = _snapshot(
        remote_branch_sha=None,
        ancestry={
            "state": "clean",
...
def test_main_only_drift_remains_integrable_after_a_prior_refresh() -> None:
    """A refreshed clean snapshot must tolerate a second unrelated main advance."""
    refreshed_baseline = _snapshot(
        base_sha="base-b",
        local_head_sha="head-after-first-merge",
        remote_branch_sha=None,
        ancestry={
            "state": "clean",
            "main_tip_sha": "base-b",
            "merge_base_sha": "base-b",
        },
    )
    second_advance = _snapshot(
        base_sha="base-c",
        local_head_sha="head-after-first-merge",
        remote_branch_sha=None,
        ancestry={
    ...
    result = gate.evaluate_state(refreshed_baseline, second_advance)
...
def test_sync_integrates_main_only_drift_despite_transient_stack_label(
    tmp_path, monkeypatch, capsys
) -> None:
    """Sync must merge advanced main when the captured feature head itself stayed unchanged."""
    baseline = _snapshot(
        remote_branch_sha=None,
        ancestry={
            "state": "clean",
    ...
    refreshed = _snapshot(
        base_sha="base-b",
        local_head_sha="head-after-merge",
        remote_branch_sha=None,
        ancestry={
            "state": "clean",
            "main_tip_sha": "base-b",
    ...
    states = iter([current, refreshed])
    integrated_targets: list[list[str]] = []
    monkeypatch.setattr(gate, "collect_live_state", lambda **_: next(states))

    def integrate(**kwargs: Any) -> dict[str, Any]:
        integrated_targets.append(kwargs["targets"])
        return {"ok": True, "merged": kwargs["targets"]}

    monkeypatch.setattr(gate, "_integrate_targets", integrate)

    assert gate.main(["sync", "--snapshot-path", str(snapshot_path), "--integrate"]) == 0

    output = json.loads(capsys.readouterr().out)
    assert integrated_targets == [["refs/remotes/origin/main"]]
    assert output["decision"] == "ready"
    assert output["reason"] == "remote_state_integrated"
    assert output["integrated"] == ["refs/remotes/origin/main"]


def test_real_git_sync_handles_two_main_advances(tmp_path, monkeypatch) -> None:
    """A real feature branch stays integrable across repeated unrelated main advances."""
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    git("init", "-b", "main")
    git("config", "user.name", "Prepublication Test")
    git("config", "user.email", "prepublication@example.invalid")
    (repo / "base.txt").write_text("base\n", encoding="utf-8")
    git("add", "base.txt")
    git("commit", "-m", "base")
    base_a = git("rev-parse", "HEAD")
    git("update-ref", "refs/remotes/origin/main", base_a)

    git("checkout", "-b", "feature/fresh-state")
    (repo / "feature.txt").write_text("feature\n", encoding="utf-8")
    git("add", "feature.txt")
    git("commit", "-m", "feature")
    feature_head = git("rev-parse", "HEAD")
    monkeypatch.chdir(repo)
    baseline = gate._record_ancestry(
        _snapshot(
            base_sha=base_a,
    ...
    git("checkout", "main")
    (repo / "main.txt").write_text("advance one\n", encoding="utf-8")
    git("add", "main.txt")
    git("commit", "-m", "advance main once")
    base_b = git("rev-parse", "HEAD")
    git("update-ref", "refs/remotes/origin/main", base_b)
    git("checkout", "feature/fresh-state")
    first_drift = gate._record_ancestry(
        _snapshot(
            base_sha=base_b,
            local_head_sha=feature_head,
            remote_branch_sha=None,
    ...
    first_decision = gate.evaluate_state(baseline, first_drift)

    assert first_drift["ancestry"]["state"] == "undeclared_stack"
    assert first_decision["decision"] == "refresh-required"
    integration = gate._integrate_targets(
        remote="origin",
        branch="feature/fresh-state",
        targets=["refs/remotes/origin/main"],
    )
    assert integration == {"ok": True, "merged": ["refs/remotes/origin/main"]}

    merged_head = git("rev-parse", "HEAD")
    refreshed = gate._record_ancestry(
        _snapshot(
            base_sha=base_b,
            local_head_sha=merged_head,
            remote_branch_sha=None,
        )
    )
    assert refreshed["ancestry"]["state"] == "clean"

    git("checkout", "main")
    (repo / "main.txt").write_text("advance two\n", encoding="utf-8")
    git("add", "main.txt")
    git("commit", "-m", "advance main twice")
    base_c = git("rev-parse", "HEAD")
    git("update-ref", "refs/remotes/origin/main", base_c)
    git("checkout", "feature/fresh-state")
    second_drift = gate._record_ancestry(
        _snapshot(
            base_sha=base_c,
            local_head_sha=merged_head,
            remote_branch_sha=None,
    ...
    second_decision = gate.evaluate_state(refreshed, second_drift)

    assert second_drift["ancestry"]["state"] == "undeclared_stack"
    assert second_decision["decision"] == "refresh-required"
    assert second_decision["reason"] == "base_changed"
