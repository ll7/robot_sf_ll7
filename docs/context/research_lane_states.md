...
| --- | --- | --- | --- | --- | --- |
| Topology selection scoring | #2624 | `revise` | [issue_2624_topology_reuse_penalty_gate.md](issue_2624_topology_reuse_penalty_gate.md) | Define a new topology progress hypothesis before any further local planner variant; the paired reuse-penalty gate activated but did not improve command influence, route progress, or terminal behavior. | Stop treating the reuse-penalty gate as benchmark improvement evidence; do not rerun the same candidate unchanged on the same slice. |
