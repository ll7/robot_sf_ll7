...
@pytest.mark.parametrize("value", [3.000001, 1e308])
def test_classify_rejects_oversized_finite_realized_speed_mean(value: float) -> None:
    """Finite realized means above the sampler ceiling fail closed."""
    payload, protocol = preflight.load_preflight()
    diagnostics = _diagnostics(payload, protocol)
    diagnostics["rows"][0]["diagnostics"]["realized_desired_speed_mean_m_s"] = value

    with pytest.raises(
        preflight.PreflightError, match="realized desired-speed mean is out of range"
    ):
        preflight.classify_activation(payload, protocol, diagnostics)


@pytest.mark.parametrize("value", [3.000001, 1e308])
def test_classify_rejects_oversized_finite_realized_speed_std(value: float) -> None:
    """Finite standard deviations above the sampler ceiling fail closed."""
    payload, protocol = preflight.load_preflight()
    diagnostics = _diagnostics(payload, protocol)
    diagnostics["rows"][0]["diagnostics"]["realized_desired_speed_std_m_s"] = value

    with pytest.raises(
        preflight.PreflightError, match="realized desired-speed std is out of range"
    ):
        preflight.classify_activation(payload, protocol, diagnostics)


def test_classify_accepts_realized_speed_std_bound() -> None:
    """The inclusive 3.0 m/s protocol ceiling remains a valid boundary value."""
    payload, protocol = preflight.load_preflight()
    diagnostics = _diagnostics(payload, protocol)
    diagnostics["rows"][0]["diagnostics"]["realized_desired_speed_std_m_s"] = 3.0
