...
class TestFeatureExtractors:
    ...
    def observation_space(self):
        ...
        return spaces.Dict({
            OBS_DRIVE_STATE: drive_state_space,
            OBS_RAYS: rays_space
        })
...
class TestFeatureExtractorConfig:
    ...
    def test_create_config_function(self):
        ...
        # Test with enum
        config2 = create_feature_extractor_config(
            FeatureExtractorType.ATTENTION,
            embed_dim=32
        )
...
class TestParameterCounting:
    ...
    def test_parameter_counts_different(self, observation_space):
        ...
        extractors = {
            "mlp_small": MLPFeatureExtractor(observation_space, ray_hidden_dims=[32], drive_hidden_dims=[16]),
            "mlp_large": MLPFeatureExtractor(observation_space, ray_hidden_dims=[256, 128], drive_hidden_dims=[64, 32]),
            "attention_small": AttentionFeatureExtractor(observation_space, embed_dim=32, num_layers=1),
            "attention_large": AttentionFeatureExtractor(observation_space, embed_dim=128, num_layers=3),
