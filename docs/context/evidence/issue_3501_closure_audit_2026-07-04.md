...
| Safety context is constructed from simulator state and requested command before execution. | PR #4137 added `compute_safety_context_from_env(...)` and runtime parser hardening in `robot_sf/benchmark/safety/safety_wrapper_runtime.py`. | Delivered. |
