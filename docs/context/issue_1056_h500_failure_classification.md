...
| `classic_t_intersection_medium` | ORCA | 111 | `exposure_enabled_completion` | `exposure_enabled_completion` | Fixed h100 times out with 9 force-exposure steps and comfort exposure 3.0; h500 succeeds at step 182 with 50 force-exposure steps, comfort exposure 16.667, and min pedestrian distance dropping from 2.235 m to 1.413 m. | Use in exposure-aware tables. Do not call it a near-miss-timing example because discrete `near_misses` remains zero. |
