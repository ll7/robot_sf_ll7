...
class Spline(Trajectory):
    ...
    def eval_spline(
        ...
        ts_normalized_nk = np.clip(ts_nk / self.final_times_n1, 0.0, 1.0)
    ...
    def rescale_velocity_and_acceleration(
        self, time_horizon_old_n1: np.ndarray, time_horizon_new_n1: np.ndarray
    ) -> None:
        """
        ...
        """
        # Convert velocities and accelerations to real world time
        time_scaling_factor_n11 = (
            time_horizon_new_n1[:, np.newaxis, :]
            / time_horizon_old_n1[:, np.newaxis, :]
        )
