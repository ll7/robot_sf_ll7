...
When the PR is opened from a worktree, verify the linked issue's fresh native Parent/Blocked
by/Blocking state. Keep `Closes`/`Refs` in `## Linked Issues` for coverage semantics; they do not
create graph edges. Review-only worktrees may inspect but must not create native links; see
