...
`diagnostic` / `proposal_preflight_only`. Completing this step proves the promotion
study is **preregistered and executable**. It is **not** evidence that TPE outperforms
random, and **not** evidence that failures transfer across planners. No paper,
dissertation, benchmark-wide, cross-planner, minimax, or portfolio claim is made or
implied.
...
Candidate/config normalization and duplicate handling are frozen (identical normalized
config collapsed and counted once per method per seed).
...
- **Missing/invalid/attrition:** fail-closed; excluded from the denominator with a
  recorded reason; no optional seeds; no outcome-dependent replacement or exclusion.
...
budget. Per the issue's explicit instruction — *"if the authorized design cannot
reasonably test that gate, declare the future run diagnostic/inconclusive here rather
than changing thresholds after outcomes"* — the future step-3 run is preregistered as
**`diagnostic_inconclusive`**. **No threshold is weakened:** the gate stays at
≥ 2 weak points, 95% interval excluding zero, and both null tests at `p ≤ 0.05`.
Claiming `promote` later would require a **re-preregistration** with more search seeds.

## Stop conditions (frozen)

Stop before step 3 if: fewer than two eligible candidates (already verified
non-triggering: 8 eligible); any evaluation outcome was inspected for this contract
before freezing; fit/evaluation lineage cannot be made disjoint; the target/reference
planner or primary mechanism cannot be pinned; the budget cannot test the positive gate
(it cannot — hence the diagnostic declaration); a row can enter without complete
...
This recomputes the contract SHA-256 and the #6139 receipt hashes, asserts every frozen
field, recomputes the permutation power math, and asserts the diagnostic declaration. It
imports no adversarial execution surface (`samplers`, `search`, `runtime`, `qd`,
`warm_start`, `transfer_matrix`, or any campaign/replay/benchmark-runner module), no
`subprocess`, and no network module; the focused test
`tests/adversarial/test_issue_5303_search_promotion_preflight.py` AST-scans the preflight
source to prove this.

## Step-3 execution contract (authoritative for #6145, not run here)

Step 3 (`https://github.com/ll7/robot_sf_ll7/issues/6145`) must register the
`constraints_first_lexicographic_v1` objective, use the corrected certifier from merged
#6139, pin the held-out `classic_group_crossing_medium` search space, and run both
samplers at budget 64 across the three frozen search seeds under the matched
simulator-time cap:
...
Expected artifacts: `output/adversarial/issue_5303_search_promotion/report.json` and
`comparison_table.md`. By this preregistration the outcome classification is
**diagnostic/inconclusive** unless re-preregistered with more seeds.
