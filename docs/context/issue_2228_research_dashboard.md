...
| Planner mechanism transfer | Protocol exists for separating slice-local mechanisms from transferable mechanisms; the first static-recentering follow-up executed and classified the pilot as `slice_local`. | `diagnostic` | The #2221 row set is underpowered and adapter-mode diagnostic evidence, but it does fail the protocol support rule for promoting recentering. | Move the next fresh mechanism-transfer effort to a clearer held-out hypothesis, currently issue [#2223](https://github.com/ll7/robot_sf_ll7/issues/2223), or explicitly pre-register a broader static-recenter expansion. | [issue_2232_planner_mechanism_transfer_benchmark.md](issue_2232_planner_mechanism_transfer_benchmark.md), [issue_2221_static_recenter_transfer.md](issue_2221_static_recenter_transfer.md), [issue_2128_heldout_scenario_family_transfer_protocol.md](issue_2128_heldout_scenario_family_transfer_protocol.md) |
...
1. Start issue [#2223](https://github.com/ll7/robot_sf_ll7/issues/2223) as the next fresh
   research-result lane now that the #2221 static-recentering transfer smoke is `slice_local`.
