...
benchmark-reset-v2-aligned Dreamer challenger profiles.
...
- The current launcher supports scenario-matrix-driven training through `env.scenario_matrix`,
  using the same YAML scenario manifests as the PPO benchmark-reset runs.
- Dreamer still remains a challenger track until benchmark evidence shows it beats the current PPO
  champion.
- Training still uses RLlib env-runner topology rather than PPO's `SubprocVecEnv`, so parity is at
  the training-curriculum level, not rollout-stack identity.
...
- writes `run_meta.json` and `resolved_config.json` alongside `run_summary.json` for reproducible
  reruns and post-hoc audit
...
- `ray.runtime_env.excludes: [...]` (for example `.git`, `.venv`, `output`, caches, large media)
...
The scenario-matrix parity profiles additionally set:

- `env.scenario_matrix.path: configs/scenarios/classic_interactions_francis2023.yaml`
- `env.scenario_matrix.strategy: random`
- `env.scenario_matrix.switch_per_reset: true`
- randomized seed semantics from `env.config.randomize_seeds: true`

The benchmark-aligned observation profiles additionally set:
...
These settings prevent worker-side uv rebuild loops, keep the run reproducible, and align Dreamer
more closely with the benchmark-facing observation contract.
...
- scenario-matrix parity now covers the training env source, but Dreamer still uses its own RLlib
  env-runner topology rather than the PPO `SubprocVecEnv` stack.
- compare results at the benchmark level, not by assuming identical rollout dynamics.
