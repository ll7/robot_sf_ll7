...
uses `rtol=0` and `atol=1e-5`. A failure reports the first divergent step and
observation field together with the maximum absolute error.
...
| `test_reset_isolation.py` | A seeded reset after another episode reproduces a fresh episode. |
...
| `test_oracle_isolation.py` | Opt-in privileged oracle transition traces do not change or enter actor-visible observations. |
