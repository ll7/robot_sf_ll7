...
def _historical_binding_locator_is_valid(locator: str) -> bool:
    ...
    if not locator.startswith("/"):
        return False
    return not any(re.search(r"~(?![01])", part) for part in locator.split("/")[1:])
...
def _validate_historical_binding_entry(  # noqa: C901, PLR0912, PLR0915
    repo_root: Path,
    binding: Any,
    index: int,
    repository: str,
    ...
    *,
    ...
    try:
        consumer_value = _load_document(repo_root / consumer_path, raw=consumer_bytes)
