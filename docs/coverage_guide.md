...
- **Gap Analysis** (formerly User Story 3): No `scripts/coverage/analyze_gaps.py` CLI or
  `robot_sf/coverage_tools/gap_analyzer.py` module exists.
- **Trend Tracking** (formerly User Story 4): No `scripts/coverage/track_trends.py` CLI,
  historical JSONL store, or coverage visualization workflow exists.
